...
void (0, _test_audit_loop_1.ensureTestRegistration)({
    ...
    goal_alignment: [{
            ...
            discrimination_claim: "Validates that submitting a composition_chain'd leaf trace propagates Δα to ancestor variant_performance_metrics — proves the cross-scoped credit-propagation path lands.",
        }],
    discrimination_claim: "Discriminates the F-V55 cross-scoped credit-propagation failure from the underlying chain-credit math.",
 ...
 */
async function readAncestorAlpha() {
    ...
    const match = res.body.recommendations.find((r) => r.template_id === ANCESTOR_DEPTH1 ||
...
async function submitLeafTrace(index) {
...
if (baselineAlpha === null) {
    console.error(`INCONCLUSIVE: Ancestor not found in top-50 /recommend results for query "${ANCESTOR_QUERY}".`);
...
if (delta >= PASS_THRESHOLD) {
    ...
    console.error('  Credit propagation may be working but at reduced efficiency (concurrent traffic?).');
    process.exit(1);
}
//# sourceMappingURL=test-18-4-7-credit-propagation.js.map