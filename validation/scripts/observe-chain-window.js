...
function dexec(cmd) {
    try {
        return (0, node_child_process_1.execSync)(`docker exec ${CONTAINER} ${cmd}`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
    }
    catch {
        return "";
    }
}
function ls(path) { return dexec(`ls ${path}`).trim().split("\n").filter(Boolean); }
function fileLines(path) { return (0, node_fs_1.existsSync)(path) ? (0, node_fs_1.readFileSync)(path, "utf-8").split("\n").filter(Boolean).length : 0; }
function sample(idx, ts) {
    ...
    let resultsByStatus = {};
    try {
        const lines = (0, node_fs_1.existsSync)(`${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync-results.jsonl`)
            ? (0, node_fs_1.readFileSync)(`${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync-results.jsonl`, "utf-8").split("\n").filter(Boolean) : [];
        for (const l of lines) {
            try {
                const o = JSON.parse(l);
                const s = o.push_status ?? "unknown";
                resultsByStatus[s] = (resultsByStatus[s] ?? 0) + 1;
            }
            catch { }
        }
    }
    ...
    let commitCount = 0;
    try {
        commitCount = Number((0, node_child_process_1.execSync)(`git -C ${REPO_ROOT} log --since="1 hour ago" --grep="substrate-authored\\|host-sync" --oneline | wc -l`, { encoding: "utf-8" }).trim());
    }
    catch { }
...
(async () => {
    ...
    for (let i = 0; i < SAMPLES; i++) {
        ...
        (0, node_fs_1.writeFileSync)(OUT, JSON.stringify({ ts: new Date().toISOString(), samples }, null, 2));
        if (i < SAMPLES - 1)
    ...
    (0, node_fs_1.writeFileSync)(OUT, JSON.stringify({ ts: new Date().toISOString(), samples, delta }, null, 2));
    console.log(`\nDELTA over ${INTERVAL_MIN * (SAMPLES - 1)} min: ${JSON.stringify(delta)}`);
})();
//# sourceMappingURL=observe-chain-window.js.map