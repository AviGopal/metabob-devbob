...
const cfg = JSON.parse((0, node_fs_1.readFileSync)(`${process.env.HOME}/.metabob/config.json`, "utf-8"));
...
function inContainer(cmd) {
    return (0, node_child_process_1.execSync)(`docker exec ${CONTAINER} ${cmd}`, { encoding: "utf-8" });
}
function listContainerDir(path) {
    try {
        return inContainer(`ls ${path}`).trim().split("\n").filter(Boolean);
    }
    catch {
        return [];
    }
}
async function dispatch(targetTemplateId, variables = {}) {
    ...
    if (body.error || !body.dispatchId)
        throw new Error(`dispatch failed: ${JSON.stringify(body)}`);
    return body.dispatchId;
}
async function poll(dispatchId, timeoutMs = 120_000) {
    ...
    while (Date.now() < deadline) {
        ...
        if (r.ok) {
            ...
            if (b.status === "completed" || b.status === "failed")
                return { status: b.status, executionId: b.executionId ?? "" };
        }
        await new Promise(r => setTimeout(r, 2000));
    }
    return { status: "timeout", executionId: "" };
}
async function resolveDev(type, body = {}) {
...
async function runStage(stage, name, runner, invariants) {
    console.log(`\n=== ${stage}: ${name} ===`);
    const runs = [];
...
async function stage2(i) {
    ...
    const r = await poll(id);
    if (r.status !== "completed")
        return { run: i, pass: false, durationMs: 0, detail: `status=${r.status}` };
...
async function stage3(i) {
    const scenario = SCENARIO_FILES[i % SCENARIO_FILES.length];
    if (!scenario)
        return { run: i, pass: false, durationMs: 0, detail: "no scenario file" };
...
async function stage4(i) {
    ...
    if (!proposal)
        return { run: i, pass: false, durationMs: 0, detail: "no proposal" };
...
async function stage5(i) {
    ...
    if (!m)
        return { run: i, pass: false, durationMs: 0, detail: `no parseable mitosis dir; have ${mitosisDirs.length}` };
...
async function stage6(i) {
    const mitosisDirs = listContainerDir("/vessels").filter(d => d.includes("mitosis"));
    const target = mitosisDirs[i % Math.max(1, mitosisDirs.length)] ?? "";
    const m = target.match(/^([\w-]+?)-mitosis-(.+)$/);
    if (!m)
        return { run: i, pass: false, durationMs: 0, detail: `no parseable mitosis dir` };
    const [, vesselName, mitosisVersion] = m;
    const intentBefore = (0, node_fs_1.existsSync)(`${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync.jsonl`) ? (0, node_fs_1.readFileSync)(`${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync.jsonl`, "utf-8").split("\n").filter(Boolean).length : 0;
    ...
    const intentAfter = (0, node_fs_1.existsSync)(`${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync.jsonl`) ? (0, node_fs_1.readFileSync)(`${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync.jsonl`, "utf-8").split("\n").filter(Boolean).length : 0;
...
async function stage7(i) {
    const resultsPath = `${REPO_ROOT}/scripts/substrate/workspace/mitosis-applied-host-sync-results.jsonl`;
    const before = (0, node_fs_1.existsSync)(resultsPath) ? (0, node_fs_1.readFileSync)(resultsPath, "utf-8").split("\n").filter(Boolean).length : 0;
    try {
        (0, node_child_process_1.execSync)(`bash ${REPO_ROOT}/scripts/substrate/host-sync-poller.sh --once`, { encoding: "utf-8", stdio: "pipe", timeout: 60_000 });
    }
    catch (e) {
        // poller can exit non-zero; we still inspect results file
    }
    const after = (0, node_fs_1.existsSync)(resultsPath) ? (0, node_fs_1.readFileSync)(resultsPath, "utf-8").split("\n").filter(Boolean).length : 0;
...
(async () => {
    ...
    reports.push(await runStage("S1", "Detection (orthogonality audit)", stage1, (runs) => {
        const counts = runs.map(r => r.data?.gapsFiles ?? 0).filter(n => n > 0);
    ...
    reports.push(await runStage("S2", "Bridge (gap → scenario)", stage2, (runs) => {
        const deltas = runs.map(r => (r.data?.after ?? 0) - (r.data?.before ?? 0));
    ...
    reports.push(await runStage("S3", "Draft (scenario → proposal)", stage3, (runs) => {
        const scenarios = runs.map(r => r.data?.scenario);
        const deltas = runs.map(r => (r.data?.after ?? 0) - (r.data?.before ?? 0));
    ...
    reports.push(await runStage("S4", "Apply (proposal → mitosis)", stage4, (runs) => {
        const created = runs.filter(r => r.data?.created).length;
    ...
    reports.push(await runStage("S5", "Evaluate (mitosis → verdict)", stage5, (runs) => {
        const verdicts = runs.map(r => r.data?.verdict);
        const byMitosis = {};
        for (const r of runs) {
            const m = r.data?.mitosisVersion;
            if (m)
                (byMitosis[m] ??= []).push(r.data?.verdict);
        }
    ...
    reports.push(await runStage("S6", "Cutover (verdict → intent)", stage6, (runs) => {
        const intents = runs.filter(r => (r.data?.intentDelta ?? 0) > 0).length;
    ...
    reports.push(await runStage("S7", "Host-sync (intent → commit)", stage7, (runs) => {
        const writes = runs.filter(r => (r.data?.delta ?? 0) > 0).length;
        return [{ name: "result lines written on processed intents", pass: writes >= 1 || runs.every(r => (r.data?.delta ?? 0) === 0), detail: `runs_with_writes=${writes}` }];
    }));
    const out = { ts: new Date().toISOString(), runs: RUNS, reports };
    (0, node_fs_1.writeFileSync)(OUT, JSON.stringify(out, null, 2));
    console.log(`\n=== SUMMARY ===`);
    for (const r of reports)
        console.log(`${r.stage} ${r.name}: ${r.passRate}; invariants: ${r.invariants.map(i => `${i.name}=${i.pass ? "OK" : "X"}`).join(", ")}`);
    console.log(`\nwrote ${OUT}`);
})().catch(e => { console.error(e); process.exit(1); });
//# sourceMappingURL=probe-chain-stages.js.map