...
function runGit(args, cwd) {
    const r = (0, node_child_process_1.spawnSync)("git", args, { cwd, encoding: "utf-8" });
    return {
        exitCode: r.status ?? -1,
        stdout: r.stdout ?? "",
        stderr: r.stderr ?? "",
...
function runResolver(resolverType, data, cwd) {
    const dataStr = JSON.stringify(data);
    const result = (0, node_child_process_1.spawnSync)("bun", [
...
async function main() {
    ...
    const expected = {
        ...
        workingTreeChanges: rawStatus.exitCode === 0
        ...
        recentCommits: rawLog.exitCode === 0 ? rawLog.stdout.trim().split("\n").filter((l) => l.length > 0) : [],
    };
    const divergences = [];
    ...
    if (vesselDiffStat.filesChanged !== expected.diffStat.filesChanged ||
    ...
    if (vesselLogLines.length !== expected.recentCommits.length ||
    ...
    if (divergences.length === 0) {
        ...
        console.log("   The development-vessel branch-health activity can substitute for manual git commands.");
        process.exit(0);
    }
    else {
...
//# sourceMappingURL=verify-development-vessel-branch-health.js.map