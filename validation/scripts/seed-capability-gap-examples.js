...
const ENTRIES = [
    {
        ...
        content: "Pattern: trace shows 'unknown shape: git_status_with_dirty_files' error " +
    ...
    },
    {
        ...
        content: "[principle__substrate_extends_its_own_capability_surface] When the " +
    ...
    },
    {
        ...
        content: "[principle__every_no_handler_is_capability_gap] Any task failure with " +
...
async function loadApiKey() {
    ...
    if (env)
        return env;
    try {
        const raw = await (0, promises_1.readFile)((0, node_path_1.join)((0, node_os_1.homedir)(), ".metabob", "config.json"), "utf-8");
        const cfg = JSON.parse(raw);
        if (typeof cfg?.metabob?.apiKey === "string")
            return cfg.metabob.apiKey;
    }
    catch { /* fall through */ }
    return "";
}
async function exists(apiKey, signature, sourceType) {
    ...
    try {
        ...
        return (j.concepts ?? []).some((c) => (typeof c.summary === "string" && c.summary.includes(signature)) ||
...
async function mint(apiKey, entry) {
    const body = {
    ...
    if (!r.ok)
        throw new Error(`mint failed ${r.status}: ${(await r.text()).slice(0, 300)}`);
    const j = await r.json();
...
async function main() {
    ...
    if (!apiKey)
        console.warn("warning: no METABOB_API_KEY — proceeding unauthenticated");
    console.log(`seeding ${ENTRIES.length} capability-gap concepts to ${CONCEPT_DB_URL}`);
    let minted = 0, skipped = 0, failed = 0;
    for (const e of ENTRIES) {
        try {
            if (await exists(apiKey, e.signature, e.source_type)) {
                console.log(`  skip ${e.name} (already present)`);
                skipped++;
                continue;
            }
            await mint(apiKey, e);
...
//# sourceMappingURL=seed-capability-gap-examples.js.map