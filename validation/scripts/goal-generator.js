...
const shape_registry_hash_ts_1 = require("./lib/shape-registry-hash.ts");
const shape_signature_pool_ts_1 = require("./lib/shape-signature-pool.ts");
const topology_gap_band_ts_1 = require("./lib/topology-gap-band.ts");
const adversarial_mutate_ts_1 = require("./lib/adversarial-mutate.ts");
// ---------------------------------------------------------------------------
...
async function loadConfig() {
    ...
    if ((0, node_fs_1.existsSync)(configPath)) {
        ...
        const discoveryEndpoint = envDiscovery ?? "https://discovery.metabob.com";
        if (apiKey)
            return { endpoint, apiKey, discoveryEndpoint };
    }
...
function formatCellId(c) {
    return `${c.novelty}|${c.depth}|${c.scenario}`;
}
function classifyNovelty(count) {
    if (count >= 5)
        return "seen";
    if (count >= 1)
        return "rare";
...
// ---------------------------------------------------------------------------
async function classifyDepth(outputShapes, endpoint, authHeaders) {
    ...
    for (const m of matches) {
        ...
        const beta = m.beta ?? 1;
        const totalExecs = m.total_executions ??
            m.sample_count ??
            Math.max(0, (alpha - 1) + (beta - 1));
        if (totalExecs >= 5) {
            for (const s of m.output_shapes ?? []) {
                if (outputShapes.includes(s))
                    coveredShapes.add(s);
            }
        }
    }
...
const CELL_TEMPLATES = [
    // seen | depth0 | A
    {
        ...
        template: (inp, out) => `Given a ${inp} as context, produce a ${out} using the established pipeline.`,
    },
    // seen | depth1 | A  (2 templates — this cell is weight-doubled intentionally)
    {
        cell: { novelty: "seen", depth: "depth1", scenario: "A" },
        template: (inp, out) => `Starting from a ${inp}, resolve any missing intermediate shapes and deliver a ${out}.`,
    },
    {
        cell: { novelty: "seen", depth: "depth1", scenario: "A" },
        template: (inp, out) => `Use the ${inp} to diagnose the issue and produce a ${out} with at least one sub-task delegation.`,
    },
    // seen | depth2+ | A  (2 templates)
    {
        cell: { novelty: "seen", depth: "depth2+", scenario: "A" },
        template: (inp, out) => `Orchestrate a multi-step workflow starting with ${inp} that eventually yields a ${out} through two or more composition layers.`,
    },
    {
        cell: { novelty: "seen", depth: "depth2+", scenario: "A" },
        template: (inp, out) => `Produce a ${out} from ${inp} context via a deep composition chain with at least two intermediate goal escalations.`,
    },
    // seen | depth0 | B  (1 template)
    {
        cell: { novelty: "seen", depth: "depth0", scenario: "B" },
        template: (inp, out) => `Generate a ${out} from ${inp} using a sparsely-tested activity path (cold Thompson posterior).`,
    },
    // seen | depth1 | B  (1 template)
    {
        cell: { novelty: "seen", depth: "depth1", scenario: "B" },
        template: (inp, out) => `Starting with ${inp}, produce ${out} through a cold-start activity that has seen fewer than three executions.`,
    },
    // seen | depth2+ | B  (1 template)
    {
        cell: { novelty: "seen", depth: "depth2+", scenario: "B" },
        template: (inp, out) => `Using ${inp} as the seed, construct a ${out} through a rarely-exercised multi-hop composition.`,
    },
    // seen | depth0 | C∪D  (1 template)
    {
        cell: { novelty: "seen", depth: "depth0", scenario: "C∪D" },
        template: (inp, out) => `Attempt to produce a ${out} from ${inp} where no existing template covers the target shape directly.`,
    },
    // seen | depth1 | C∪D  (1 template)
    {
        cell: { novelty: "seen", depth: "depth1", scenario: "C∪D" },
        template: (inp, out) => `Resolve ${inp} and escalate to create a ${out} even though the shape has no matching activity template.`,
    },
    // rare | depth0 | A  (1 template)
    {
        cell: { novelty: "rare", depth: "depth0", scenario: "A" },
        template: (inp, out) => `Produce a ${out} from ${inp} — this shape pair has been seen in only a few prior traces.`,
    },
    // rare | depth1 | A  (1 template)
    {
        cell: { novelty: "rare", depth: "depth1", scenario: "A" },
        template: (inp, out) => `Starting from ${inp}, derive a ${out} via one composition layer; this shape signature is infrequent in the trace history.`,
    },
    // rare | depth2+ | A  (1 template)
    {
        cell: { novelty: "rare", depth: "depth2+", scenario: "A" },
        template: (inp, out) => `Produce a ${out} from ${inp} through a rarely-observed multi-hop composition.`,
    },
    // rare | depth0 | B  (1 template)
    {
        cell: { novelty: "rare", depth: "depth0", scenario: "B" },
        template: (inp, out) => `Rename the helper in ${inp} to produce a ${out}, exercising a cold-posterior template with low execution count.`,
    },
    // rare | depth1 | B  (1 template)
    {
        cell: { novelty: "rare", depth: "depth1", scenario: "B" },
        template: (inp, out) => `Using ${inp} context, generate a ${out} via a cold single-escalation path.`,
    },
    // rare | depth2+ | B  (1 template)
    {
        cell: { novelty: "rare", depth: "depth2+", scenario: "B" },
        template: (inp, out) => `From ${inp}, produce a ${out} through a two-layer cold composition that hasn't warmed yet.`,
    },
    // rare | depth0 | C∪D  (1 template)
    {
        cell: { novelty: "rare", depth: "depth0", scenario: "C∪D" },
        template: (inp, out) => `Produce a ${out} from ${inp} where the shape is registered in a vessel but not yet backed by an activity template.`,
    },
    // rare | depth1 | C∪D  (1 template)
    {
        cell: { novelty: "rare", depth: "depth1", scenario: "C∪D" },
        template: (inp, out) => `Using ${inp} as input, create-shape-provider-goal to obtain a ${out} that has no template but an advertising vessel.`,
    },
    // novel | depth0 | A  (1 template)
    {
        cell: { novelty: "novel", depth: "depth0", scenario: "A" },
        template: (inp, out) => `Produce a ${out} from ${inp} — this exact shape combination has never appeared in prior execution traces.`,
    },
    // novel | depth1 | A  (1 template)
    {
        cell: { novelty: "novel", depth: "depth1", scenario: "A" },
        template: (inp, out) => `Combine ${inp} with a sub-goal escalation to generate a ${out} for the first time.`,
    },
    // novel | depth2+ | A  (1 template)
    {
        cell: { novelty: "novel", depth: "depth2+", scenario: "A" },
        template: (inp, out) => `Using ${inp} as the origin, traverse two or more composition hops to produce a novel ${out}.`,
    },
    // novel | depth0 | C∪D  (1 template)
    {
        cell: { novelty: "novel", depth: "depth0", scenario: "C∪D" },
        template: (inp, out) => `Forge a ${out} from ${inp} — the shape has never been produced and may require a new activity or vessel.`,
    },
    // novel | depth1 | C∪D  (1 template)
    {
        cell: { novelty: "novel", depth: "depth1", scenario: "C∪D" },
        template: (inp, out) => `Starting from ${inp}, escalate through create-shape-provider-goal to produce a novel ${out} that is absent from the registry.`,
    },
    // novel | depth2+ | C∪D  (1 template)
    {
        cell: { novelty: "novel", depth: "depth2+", scenario: "C∪D" },
        template: (inp, out) => `Orchestrate a deep novel composition from ${inp} to ${out} — gated on Phase 22 forge capability.`,
    },
];
// ---------------------------------------------------------------------------
...
if (CELL_TEMPLATES.length !== 24) {
    console.warn(`[goal-generator] Warning: expected 24 cell templates, found ${CELL_TEMPLATES.length}`);
}
// ---------------------------------------------------------------------------
...
async function generateGoals(opts) {
    ...
    let snapshotHash;
    try {
        snapshotHash = await (0, shape_registry_hash_ts_1.computeShapeRegistryHash)(opts.discoveryEndpoint, authHeaders);
        ...
        snapshotHash = (0, node_crypto_1.createHash)("sha256").update("fallback").digest("hex");
    }
    console.log(`Building shape signature pool from ${opts.endpoint}...`);
    let pool;
    try {
        pool = await (0, shape_signature_pool_ts_1.buildShapeSignaturePool)(opts.endpoint, authHeaders);
    ...
    while (goals.length < opts.count && attemptCount < MAX_ATTEMPTS) {
        ...
        const novelty = nv < noveltyThresh[0] ? "seen" : nv < noveltyThresh[1] ? "rare" : "novel";
        const dv = rng.nextFloat();
        const depth = dv < depthThresh[0] ? "depth0" : dv < depthThresh[1] ? "depth1" : "depth2+";
        const sv = rng.nextFloat();
        const scenario = sv < scenarioThresh[0] ? "A" : sv < scenarioThresh[1] ? "B" : "C∪D";
        ...
        if (novelty === "novel") {
            ...
            if (allInputShapes.length === 0)
                allInputShapes.push("file");
            if (allOutputShapes.length === 0)
                allOutputShapes.push("fileEdit");
        ...
        const matchingTemplates = CELL_TEMPLATES.filter((ct) => ct.cell.novelty === novelty &&
            ct.cell.depth === depth &&
        ...
        const inputShape = candidate.input_shapes.length > 0 ? rng.pick(candidate.input_shapes) : "file";
        const outputShape = candidate.output_shapes.length > 0 ? rng.pick(candidate.output_shapes) : "fileEdit";
    ...
    if (opts.adversarialFraction > 0 && opts.anthropicApiKey) {
        ...
        if (adversarialCount > 0) {
            ...
            for (let i = 0; i < goals.length && mutated < adversarialCount; i += step) {
                try {
                    result[i] = await (0, adversarial_mutate_ts_1.mutateGoal)(goals[i], { anthropicApiKey: opts.anthropicApiKey });
                    mutated++;
                }
                catch (err) {
...
async function main() {
    ...
    function weeklyHeldOutSeed() {
        ...
        const hash = (0, node_crypto_1.createHash)("sha256").update(label).digest();
    ...
    function parseMix(s) {
        ...
        if (parts.length !== 3)
            throw new Error(`Invalid mix "${s}": expected 3 comma-separated values`);
    ...
    if (adversarialFraction > 0) {
        ...
        if (!anthropicApiKey)
            console.warn("  Warning: ANTHROPIC_API_KEY not set; adversarial mutations will be skipped.");
    }
    ...
    let outputPath;
    if (values["output"]) {
        outputPath = (0, node_path_1.resolve)(values["output"]);
    }
    else {
        const dateStr = new Date().toISOString().slice(0, 10);
        outputPath = isHeldOut
            ? (0, node_path_1.join)(generatedDir, `${dateStr}-held-out-goals.json`)
            : (0, node_path_1.join)(generatedDir, `${seed}-${dateStr}.json`);
    }
    ...
    await (0, promises_1.writeFile)(outputPath, JSON.stringify(output, null, 2), "utf8");
...
function computeCellDistribution(goals) {
    const dist = {};
...
//# sourceMappingURL=goal-generator.js.map