...
function apiKey() {
    try {
        const c = JSON.parse((0, node_fs_1.readFileSync)(`${process.env.HOME}/.metabob/config.json`, "utf8"));
        return c.apiKey ?? c.metabob?.apiKey ?? "";
    }
    catch {
        return "";
    }
...
async function measureGroundTruth() {
    const errors = [];
    const g = { registryShapeCount: null, registryVesselCount: null, openGapCount: null, errors };
    try {
        ...
        errors.push(`registry/stats: ${e.message}`);
    }
    try {
        ...
        errors.push(`substrateGap: ${e.message}`);
    }
    return g;
}
function ladder() {
    const all = [
        {
            ...
            transformations: 1,
            nonCollapsible: "BASELINE, and deliberately collapsible — one producer whose output shape is the target shape. " +
        ...
        },
        {
            ...
            transformations: 2,
            nonCollapsible: "The count does not exist as a stored value in the note shape, and no single activity's input " +
        ...
        },
        {
            ...
            transformations: 3,
            nonCollapsible: "Two distinct producers emit two distinct shapes from two different vessels (discovery and " +
        ...
        },
        {
            ...
            transformations: 4,
            nonCollapsible: "Rung 3's two producers and comparison, plus a derived quantity (the ratio) that neither " +
...
async function dispatch(goal) {
    try {
        ...
        const j = (await r.json());
        return { dispatchId: j.dispatchId ?? null, coalesced: j.coalesced === true, err: undefined };
    }
    catch (e) {
        return { dispatchId: null, coalesced: false, err: e.message };
    }
}
/**
...
async function fetchGoalPath(goalText) {
    try {
...
async function verifySideEffect(titleContains, expectValues) {
    try {
        const r = await fetch(`${DEV_VESSEL}/v2/impulses/resolve`, {
            method: "POST", headers: { "Content-Type": "application/json" },
        ...
        if (notes.length === 0)
            return { found: false, carries: false };
 ...
 */
async function runArm(rungs, arm, g) {
    ...
    for (const s of dispatched)
        console.log(`  dispatched ${s.r.id} -> ${s.d.dispatchId?.slice(0, 8) ?? "FAILED"}${s.d.coalesced ? " (COALESCED)" : ""}`);
    return Promise.all(dispatched.map((s) => scoreOne(s.r, arm, g, s.d, s.t0)));
}
async function scoreOne(r, arm, g, d, t0) {
    const out = {
    ...
    if (!d.dispatchId) {
        out.error = "no dispatchId returned";
        return out;
    }
    ...
    if (d.coalesced && arm === "floor")
        out.error = "COALESCED on the floor arm — goal text collided; reading is void";
    const rec = await poll(d.dispatchId);
    out.latencyMs = Date.now() - t0;
    if (!rec) {
        out.error = `${out.error ? out.error + "; " : ""}never appeared in activeDispatches within ${POLL_TIMEOUT_S}s`;
        return out;
    }
    ...
    if (gp) {
        ...
        out.producerSteps = steps.n;
        out.pathActivities = steps.list;
...
async function main() {
    ...
    if (g.errors.length)
        console.log(`  ⚠ ${g.errors.length} ground-truth source unreadable — affected rungs report UNDECIDABLE, never false`);
    const rungs = ladder();
    const ctls = controls();
    const results = [];
    ...
    const sigs = floor.map((r) => r.pathSignature).filter((s) => !!s);
...
await main();
//# sourceMappingURL=complexity-ladder-harness.js.map