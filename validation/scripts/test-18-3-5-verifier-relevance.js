...
void (0, _test_audit_loop_1.ensureTestRegistration)({
    ...
    goal_alignment: [{
            ...
            discrimination_claim: "Validates that a verifier_negative trace increments impulse_relevance_metrics.times_failed, the per-impulse failure-attribution signal Thompson β updates feed on.",
        }],
    discrimination_claim: "Tests would silently pass if writeImpulseRelevancePenalty stopped updating times_failed (the documented UPDATE-not-UPSERT regression).",
...
const preCheck = await apiGet(`/v2/activities/impulse-relevance?impulse_id=${encodeURIComponent(SENTINEL_IMPULSE_ID)}`);
...
const preMetrics = preCheck.body?.metrics ?? [];
const preRow = preMetrics.find((m) => m.impulse_id === SENTINEL_IMPULSE_ID);
const initialTimesFailed = preRow?.times_failed ?? 0;
...
const postSeedCheck = await apiGet(`/v2/activities/impulse-relevance?impulse_id=${encodeURIComponent(SENTINEL_IMPULSE_ID)}`);
const postSeedMetrics = postSeedCheck.body?.metrics ?? [];
const postSeedRow = postSeedMetrics.find((m) => m.impulse_id === SENTINEL_IMPULSE_ID);
const baselineTimesFailed = postSeedRow?.times_failed ?? 0;
...
function buildFailingTrace(runId) {
...
const postCheck = await apiGet(`/v2/activities/impulse-relevance?impulse_id=${encodeURIComponent(SENTINEL_IMPULSE_ID)}`);
...
const postMetrics = postCheck.body?.metrics ?? [];
const postRow = postMetrics.find((m) => m.impulse_id === SENTINEL_IMPULSE_ID);
console.log('');
console.log('=== Results ===');
...
const actualTimesFailed = postRow?.times_failed ?? 0;
...
//# sourceMappingURL=test-18-3-5-verifier-relevance.js.map