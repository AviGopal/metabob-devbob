...
// ---------------------------------------------------------------------------
async function step_register_template() {
    const body = {
        id: FORGED_TEMPLATE_ID,
        name: "JSON Schema Validation (forged)",
        description: "Forged vessel activity: validates JSON objects against a JSON Schema produced by forge-json-schema-validator. Tagged for registry-quality audit parity test (22.7.8).",
...
// ---------------------------------------------------------------------------
async function step_write_failure_traces() {
    let written = 0;
    for (let i = 0; i < 10; i++) {
...
// ---------------------------------------------------------------------------
async function step_audit_report_no_exclusion() {
    ...
    const found = templates.some((t) => t.id === FORGED_TEMPLATE_ID || t.id?.includes("forged-vessel"));
...
// ---------------------------------------------------------------------------
async function step_vessel_metrics_degraded() {
    const res = await apiGet(`/v2/vessels/${FORGED_VESSEL_ID}/metrics?window=1h`);
    if (res.status === 404) {
...
// ---------------------------------------------------------------------------
async function step_metrics_ranking_no_exclusion() {
    const res = await apiPost("/v2/impulses/resolve", {
        pointer: {
...
async function main() {
    ...
    void (0, _test_audit_loop_1.ensureTestRegistration)({
        ...
        goal_alignment: [{
                ...
                discrimination_claim: "Validates that registry-quality tooling (core-activity-audit, replace-activity, vessel metrics) treats a forged vessel's activity exactly like any other — no forge-specific maintenance code path.",
            }],
        discrimination_claim: "Discriminates the reuse-without-special-case property: a regression introducing forge-specific exclusion would break this test while leaving the general audit path green.",
    ...
    console.log("\nConclusion:");
    console.log("  - Forged vessel activity template accepted by activity-api with no special routing");
    console.log("  - Failure traces accepted for forged vessel_id — Thompson updates apply normally");
    console.log("  - templateAuditReport ran without error — no forge exclusion in audit path");
    console.log("  - activityTemplatesByMetrics ran without error — no forge exclusion in ranking path");
    console.log("  - vessel metrics endpoint accepts forged vessel IDs — reliability metric works");
    console.log("\nThe existing registry-quality six-pack (core-activity-audit, replace-activity,");
    console.log("repair-failed-activity) will act on forged vessel activities exactly like any");
...
//# sourceMappingURL=test-22-maintenance-reuse.js.map