...
const VESSEL_GOAL = "A vessel that validates JSON objects against a provided JSON Schema and returns structured validation results including which properties failed and why.";
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
async function test_22_7_1_forge_step() {
    ...
    try {
        ...
        if (success) {
            const content = vesselVerified?.content;
            // Prefer endpoint from vesselVerified, fall back to vesselDeployedToCanary
            const endpoint = content?.endpoint ?? vesselDeployed?.content?.endpoint;
...
// ---------------------------------------------------------------------------
async function test_22_7_2_path_a_binding(vesselEndpoint) {
...
// ---------------------------------------------------------------------------
async function test_22_7_3_path_b(vesselEndpoint) {
...
// ---------------------------------------------------------------------------
async function test_22_7_4_path_c(vesselEndpoint) {
...
// ---------------------------------------------------------------------------
async function test_22_7_5_path_d(vesselEndpoint) {
...
// ---------------------------------------------------------------------------
async function test_22_7_7_path_f(vesselEndpoint) {
...
async function main() {
        ...
        goal_alignment: [
            {
                ...
                discrimination_claim: "Tests that VesselForgeHost can stand up a new vessel that immediately resolves a previously-missing shape via the discovery contract.",
            },
            {
                criterion: "#5-composition-via-features",
                discrimination_claim: "Exercises the forge → discovery → dispatch chain, three of the system's composition features in sequence.",
            },
        ],
        discrimination_claim: "Calls VesselForgeHost directly (not via slot-binding escalation); test-forge-goal-completion exercises the user-level surface for the same path.",
    ...
    if (forge.success) {
        ...
        const pf = Bun.spawn(["kubectl", "port-forward", "-n", "activity-system", "svc/forge-json-schema-validator", `${LOCAL_PORT}:8080`], { stdout: "ignore", stderr: "ignore" });
...
//# sourceMappingURL=test-22-forge-and-paths.js.map