...
const goal_host_1 = require("../../repos/ias-executor-ts/src/examples/goal-host");
class AnthropicLLM {
    apiKey;
...
async function main() {
    ...
    const events = [];
    const host = new goal_host_1.GoalHost({
        ...
        eventSink: {
            emit(event) {
                ...
                if (event.type.startsWith("lifecycle:")) {
                    ...
                    console.log(`[event] ${event.type} template=${data.templateId ?? "?"} task=${data.taskId ?? "?"} exec=${data.executionId ?? "?"}`);
                }
                ...
                if (event.type === "activity.failed") {
                    ...
                    const data = event.data;
                    console.log(`[event] activity.failed exec=${data.executionId} template=${data.templateId} error=${data.error ?? "?"}`);
                }
            },
        },
    ...
    for (const t of result.trace.tasks) {
        console.log(`[result]   ${t.taskId} (${t.resolverId}): success=${t.success} inputs=${t.inputImpulseIds.length} outputs=${t.outputImpulseIds.length}`);
    }
    ...
    if (startedCount < 2) {
        console.warn(`[verify-slot-binding] WARN: only ${startedCount} activity.started event(s); ` +
...
//# sourceMappingURL=test-slot-binding-chain.js.map