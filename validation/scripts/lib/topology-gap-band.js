...
exports.classifyTopologyGap = classifyTopologyGap;
/**
 ...
 */
async function classifyTopologyGap(shapes, activityEndpoint, discoveryEndpoint, authHeaders) {
    if (shapes.length === 0)
        return "A";
    ...
    const shapesWithTemplate = new Set();
    const shapesWithColdTemplate = new Set();
    for (const match of matches) {
        const outputShapes = match.output_shapes ?? [];
        const alpha = match.alpha ?? 1;
        const beta = match.beta ?? 1;
        const totalExecs = match.total_executions ??
            match.sample_count ??
        ...
        for (const shape of outputShapes) {
            if (shapes.includes(shape)) {
                ...
                if (isCold)
                    shapesWithColdTemplate.add(shape);
            }
        }
    }
    ...
    if (shapesWithoutTemplate.length === 0) {
        ...
        if (shapesWithColdTemplate.size > 0)
            return "B";
        return "A";
    }
...
//# sourceMappingURL=topology-gap-band.js.map