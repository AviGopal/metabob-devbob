 ...
 */
function betaCiWidth(alpha, beta) {
    const ab = alpha + beta;
    if (ab <= 0)
        return 1.0;
...
function totalExecutions(alpha, beta) {
...
// ---------------------------------------------------------------------------
async function evaluateBenchmark(entries, endpoint, authHeaders, limit) {
    const results = [];
    for (const entry of entries) {
        process.stdout.write(`  [${entry.id}] "${entry.goal_text.slice(0, 60)}..." `);
        let recommendations = [];
        try {
            const data = await apiPost(`${endpoint}/v2/activities/recommend`, authHeaders, { task_description: entry.goal_text, limit });
...
async function evaluateSearchBenchmark(entries, endpoint, authHeaders, limit) {
    const results = [];
    for (const entry of entries) {
        ...
        try {
            ...
            const data = await apiGet(url, authHeaders);
...
async function captureThompsonSnapshot(endpoint, authHeaders) {
    ...
    for (const q of broadQueries) {
        ...
        let data;
        try {
            data = await apiPost(`${endpoint}/v2/activities/recommend`, authHeaders, { task_description: q, limit: 20 });
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            if (msg.includes("budget exhausted"))
...
// ---------------------------------------------------------------------------
async function captureTraceStats(endpoint, authHeaders) {
    let traces = [];
    try {
        const data = await apiGet(`${endpoint}/v2/activities/execution-traces?limit=200`, authHeaders);
...
// ---------------------------------------------------------------------------
async function captureImproviseHealth(endpoint, authHeaders, windowTraces) {
    ...
    const improviseExecIds = new Set(improviseTraces
    ...
    let ribosomeActivations = windowTraces.filter((t) => {
        ...
        if (!parentId || !improviseExecIds.has(parentId))
            return false;
        const aid = (t.activity_id ?? "").toLowerCase();
    ...
    for (const execId of improviseExecIds) {
        if (extraCalls >= 5 || apiCallCount >= API_CALL_LIMIT - 20)
            break;
        checkedIds.add(execId);
        try {
            const data = await apiGet(`${endpoint}/v2/activities/execution-traces?parent_execution_id=${encodeURIComponent(execId)}&limit=10`, authHeaders);
            extraCalls++;
...
// ---------------------------------------------------------------------------
async function captureResolverCoverage(endpoint, authHeaders, windowTraces) {
    ...
    for (const t of toSample) {
        ...
        if (!traceId)
            continue;
        if (apiCallCount >= API_CALL_LIMIT - 25)
            break;
        try {
            const raw = await apiGet(`${endpoint}/v2/activities/execution-traces/${encodeURIComponent(traceId)}`, authHeaders);
            const full = raw.trace ?? raw.data ?? raw;
            if (full?.tasks) {
    ...
    for (const task of allTasks) {
        ...
        if (tier === "llm" || resolverId === "llm") {
            ...
            if (resolverId !== "llm" && resolverId !== "unknown")
                detCount++;
        }
...
// ---------------------------------------------------------------------------
function computeReuseTrajectory(windowTraces, thompsonSnapshot) {
...
function welchTTest(alpha1, beta1, n1, alpha2, beta2, n2) {
    if (n1 < 2 || n2 < 2)
        return { t_stat: 0, df: 0, p_value: 1 };
...
async function computeDiscriminationStat(endpoint, authHeaders) {
    ...
    for (const [templateId, buckets] of byTemplate) {
        ...
        if (!b)
            continue;
        const { t_stat, df, p_value } = welchTTest(a.alpha, a.beta, a.n_observations, b.alpha, b.beta, b.n_observations);
        if (p_value < 0.05) {
            discriminatingCount++;
...
// ---------------------------------------------------------------------------
async function computeExecutabilityScores(entries, thompsonSnapshot, endpoint, authHeaders, detailed) {
    if (!detailed) {
    ...
    for (const entry of entries) {
        ...
        try {
            const raw = await apiGet(`${endpoint}/v2/activities/templates/${encodeURIComponent(entry.expected_activity_id)}`, authHeaders);
            templateData = raw;
            extraCalls++;
        }
        catch {
        ...
        const tpl = templateData.template ?? templateData.data ?? templateData;
...
function computeMetrics(entries) {
    if (entries.length === 0)
        return { mrr: 0, hit_at_1: 0, hit_at_3: 0, hit_at_5: 0 };
...
// Print summary table
// ---------------------------------------------------------------------------
function printSummary(report, baseline, benchmarkEntries) {
    const sep = "=".repeat(88);
    console.log(`\n${sep}`);
    ...
    const delta = (curr, prev) => prev === undefined ? "" : ` (${curr >= prev ? "+" : ""}${((curr - prev) * 100).toFixed(1)}pp)`;
    console.log(`\nMetrics:`);
    console.log(`  MRR      : ${fmt(report.mrr)}${baseline ? delta(report.mrr, baseline.mrr) : ""}`);
    console.log(`  Hit@1    : ${pct(report.hit_at_1)}${baseline ? delta(report.hit_at_1, baseline.hit_at_1) : ""}`);
    console.log(`  Hit@3    : ${pct(report.hit_at_3)}${baseline ? delta(report.hit_at_3, baseline.hit_at_3) : ""}`);
    console.log(`  Hit@5    : ${pct(report.hit_at_5)}${baseline ? delta(report.hit_at_5, baseline.hit_at_5) : ""}`);
    ...
    if (!rc || rc.sampled_traces === 0) {
        ...
        console.log(`  Resolver coverage : llm=${pct(rc.llm_tier_rate)}, det=${pct(rc.deterministic_rate)}, pat=${pct(rc.pattern_rate)}  (top: ${topNames || "none"})  [llm ↓ is good]`);
    }
    ...
    if (!rt) {
        ...
        console.log(`  Reuse trajectory  : reuse_rate=${pct(rt.reuse_rate)}  (mean_depth=${rt.mean_composition_depth.toFixed(1)})  [↑ is good]`);
    ...
    const COL_CAT = 14;
    const COL_GOAL = 44;
    console.log(`${"id".padEnd(COL_ID)}${"category".padEnd(COL_CAT)}${"goal (truncated)".padEnd(COL_GOAL)}  rank  RR`);
    ...
    for (const e of report.entries) {
        ...
        console.log(`${e.id.padEnd(COL_ID)}${e.category.padEnd(COL_CAT)}${goalText.padEnd(COL_GOAL)}  ${e.found ? String(e.rank).padStart(4) : "  NF"}  ${e.rr.toFixed(3)}`);
    }
    console.log(`\nThompson snapshot (top-10 by EV):`);
    const top10 = report.thompson_snapshot.slice(0, 10);
    console.log(`${"activity".padEnd(48)}  ${"α".padStart(7)}  ${"β".padStart(7)}  ${"EV".padStart(7)}  ${"CI±".padStart(8)}  executions`);
    console.log("-".repeat(100));
    for (const t of top10) {
        const name = t.activity_id.slice(0, 47).padEnd(48);
        console.log(`${name}  ${t.alpha.toFixed(1).padStart(7)}  ${t.beta.toFixed(1).padStart(7)}  ${t.ev.toFixed(4).padStart(7)}  ${t.ci_width.toFixed(4).padStart(8)}  ${t.total_executions}`);
    }
 ...
 */
async function fetchAuditSummary(endpoint, authHeaders) {
    ...
    if (!auditBody.success || !auditBody.content)
        return undefined;
    const auditContent = JSON.parse(auditBody.content);
    const entries = auditContent.entries ?? [];
    let passed = 0;
    let passedWithCaveat = 0;
    const failedBySubtype = {};
    const caveats = {};
    for (const entry of entries) {
        const entryPassed = entry.passed === true;
        const entryCaveats = Array.isArray(entry.caveats) ? entry.caveats : [];
        for (const cv of entryCaveats) {
            caveats[cv] = (caveats[cv] ?? 0) + 1;
        }
        if (entryPassed) {
            passed++;
            if (entryCaveats.length > 0)
                passedWithCaveat++;
        }
        else {
            const failedEvidence = entry.failed_evidence;
    ...
    try {
        ...
        if (propResp.ok) {
            ...
            if (propBody.success && propBody.content) {
                const propContent = JSON.parse(propBody.content);
                const props = propContent.entries ?? [];
                const supersededIds = new Set();
                for (const p of props) {
                    const sup = Array.isArray(p.supersedes) ? p.supersedes : [];
                    for (const id of sup)
                        supersededIds.add(id);
                }
                for (const p of props) {
                    if (typeof p.id === "string" && !supersededIds.has(p.id))
                        openProposals++;
                }
            }
        }
...
async function main() {
    ...
    if (!benchmarkArg || benchmarkArg === "v1") {
        benchmarkPath = (0, node_path_1.join)(repoRoot, "validation", "activity-reuse-benchmark.json");
        benchmarkVersion = "v1";
    }
    else if (benchmarkArg === "v2") {
        benchmarkPath = (0, node_path_1.join)(repoRoot, "validation", "activity-reuse-benchmark-v2.json");
        benchmarkVersion = "v2";
    }
    else {
        // Treat as explicit file path (absolute or relative to CWD)
        benchmarkPath = (0, node_path_1.resolve)(benchmarkArg);
        benchmarkVersion = "custom";
    }
    if (!(0, node_fs_1.existsSync)(benchmarkPath)) {
        throw new Error(`Benchmark file not found: ${benchmarkPath}`);
    }
    const benchmarkEntries = JSON.parse(await (0, promises_1.readFile)(benchmarkPath, "utf8"));
    console.log(`\nBenchmark: ${benchmarkVersion} (${benchmarkPath})`);
    console.log(`\nLoaded ${benchmarkEntries.length} benchmark entries from ${benchmarkPath}`);
    // Load baseline for comparison
    let baseline = null;
    if (baselineDate) {
        const basePath = (0, node_path_1.join)(resultsDir, `${baselineDate}-reuse-report.json`);
        if ((0, node_fs_1.existsSync)(basePath)) {
            baseline = JSON.parse(await (0, promises_1.readFile)(basePath, "utf8"));
    ...
    let traceStats = { sample_size: 0, improvise_count: 0, improvise_rate: 0, window_days: 7 };
    let windowTraces = [];
    try {
    ...
    const searchMetrics = searchResults.length > 0 ? computeMetrics(searchResults.map((e) => ({ ...e, rank: e.search_rank ?? 0, rr: e.search_rr ?? 0, found: e.search_found ?? false }))) : null;
    ...
    const quadrantCounts = mergedResults.reduce((acc, e) => {
        if (e.diagnostic && e.diagnostic !== null)
            acc[e.diagnostic]++;
        return acc;
    }, { A: 0, B: 0, C: 0, D: 0 });
    ...
    try {
        enrichedResults = await computeExecutabilityScores(mergedResults, thompsonSnapshot, endpoint, authHeaders, detailed);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        if (msg.includes("budget exhausted")) {
    ...
    const outputPath = (0, node_path_1.join)(resultsDir, `${isoDate}${labelSlug}${benchSlug}-reuse-report.json`);
            ...
            goal_alignment: [
                {
                    ...
                    discrimination_claim: "Tracks whether activity-recommend quality (MRR + hit@k) improves after each Phase 18+ change. A regression in Thompson posterior writes, dense-search ranking, or impulse-relevance updates flips the MRR signal.",
                },
                {
                    criterion: "#6-reuse-up-improvise-down",
                    discrimination_claim: "Reuse trajectory + improvise-rate metrics directly measure the system's bias toward reusing known activities over improvising.",
                },
            ],
            discrimination_claim: "Harness regression is detectable as Δ recommend_mrr or Δ search_mrr beyond the gate thresholds in run-weekly-harness.sh.",
...
//# sourceMappingURL=reuse-harness.js.map