...
async function loadConfig() {
    ...
    let apiKey = envKey;
    let discovery = envDiscovery;
    let conceptDb = envConcept;
    ...
    if ((0, node_fs_1.existsSync)(configPath)) {
        try {
            ...
            console.warn(`[narrator] Could not parse ${configPath}: ${err.message}`);
        }
    }
    if (!endpoint) {
        throw new Error("Activity-API endpoint not set. Set METABOB_ENDPOINT env or metabob.endpoint in ~/.metabob/config.json");
    }
    if (!apiKey) {
        throw new Error("METABOB_API_KEY not set. Set via env var or metabob.apiKey in ~/.metabob/config.json");
    }
    ...
    for (const arg of process.argv.slice(2)) {
        ...
        if (m)
            snapshotIntervalMs = Number.parseInt(m[1], 10);
    }
    ...
    const outputDir = (0, node_path_1.resolve)(here, "..", "observations");
    return {
...
async function fetchTemplates(endpoint, apiKey) {
...
async function fetchDiscoveryStats(endpoint, apiKey) {
    if (!endpoint) {
    ...
    try {
        ...
        return { endpoint, reachable: false, stats: null, error: err.message };
    }
}
async function fetchConceptInventory(endpoint, apiKey) {
    if (!endpoint) {
        return {
            endpoint: null,
            reachable: false,
...
async function fetchRecentTraces(endpoint, apiKey) {
...
async function captureSnapshot(cfg) {
    ...
    const snapDir = (0, node_path_1.join)(cfg.outputDir, "snapshots");
    ensureDir(snapDir);
    const snapPath = (0, node_path_1.join)(snapDir, `snapshot-${stamp}.json`);
    const tracePath = (0, node_path_1.join)(cfg.outputDir, `recent-traces-${stamp}.json`);
    await (0, promises_1.writeFile)(snapPath, JSON.stringify(snapshot, null, 2));
    await (0, promises_1.writeFile)(tracePath, JSON.stringify(traces, null, 2));
    console.log(`[narrator] snapshot ${captured_at} — templates=${snapshot.activity_api.template_count ?? "?"} concepts=${snapshot.concept_db.concept_count ?? "?"} discovery=${snapshot.discovery_vessel.reachable ? "ok" : "n/a"} → ${snapPath}`);
}
// ---------------------------------------------------------------------------
...
class NarratorObserver {
    ...
    constructor(cfg) {
        ...
        this.eventLogPath = (0, node_path_1.join)(cfg.outputDir, `events-${todayDate()}.jsonl`);
    }
    start() {
        this.shouldRun = true;
        this.connect();
    ...
    stop() {
        ...
        if (this.ws) {
            try {
                ...
                console.warn(`[narrator] close error: ${err.message}`);
            }
            this.ws = null;
        }
    }
    ...
    connect() {
        ...
        try {
            ...
            console.warn(`[narrator] WS construct failed: ${err.message}`);
        ...
        ws.addEventListener("open", () => {
            ...
            try {
                ...
                console.warn(`[narrator] auth send failed: ${err.message}`);
            }
        });
        ws.addEventListener("message", (evt) => {
            try {
                ...
                console.warn(`[narrator] event parse failed: ${err.message}`);
            }
        });
        ...
        ws.addEventListener("error", (evt) => {
            console.warn(`[narrator] WS error: ${evt.message ?? "unknown"}`);
        });
    }
    async handleEvent(event) {
        ...
        if (!CAPTURED_EVENT_TYPES.has(event.type))
            return;
        const enriched = {
        ...
        try {
            await (0, promises_1.appendFile)(this.eventLogPath, JSON.stringify(enriched) + "\n");
        }
        catch (err) {
            console.warn(`[narrator] event write failed: ${err.message}`);
            return;
        }
        // Brief stdout line for human watching.
        const data = event.data ?? {};
        const succinct = event.type === "task.completed"
            ? `success=${data.success} task=${data.task_id}`
            : event.type === "task.failed"
                ? `task=${data.task_id} error=${data.error?.slice(0, 60)}`
...
async function main() {
    ...
    ensureDir(cfg.outputDir);
    ensureDir((0, node_path_1.join)(cfg.outputDir, "snapshots"));
    ...
    await captureSnapshot(cfg).catch((err) => console.warn(`[narrator] snapshot failed: ${err.message}`));
    const snapshotTimer = setInterval(() => {
        captureSnapshot(cfg).catch((err) => console.warn(`[narrator] snapshot failed: ${err.message}`));
...
main().catch((err) => {
    console.error(`[narrator] fatal: ${err.message}`);
    process.exit(1);
});
//# sourceMappingURL=substrate-narrator.js.map