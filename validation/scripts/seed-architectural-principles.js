...
const HAND_LISTED_PRINCIPLES = [
        ...
        check_hints: [
            {
                ...
                forbidden_pattern_regex: "v2/activities/templates\\?|reuseList|reuse_template|LLM[-_]?reuse|selectBestTemplate",
                detail: "goal-host fetches template catalogue + does LLM-reuse — both should " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "structural",
        summary: "Single LLM-capable dispatcher = single point of failure for autonomous self-modification.",
        content: "With only goal-host-vessel as the LLM-capable execution path, any " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "structural",
        summary: "Per-dispatch full-state capture (state_signature + ProxyImpulseBus + recommend pre-check) = O(N) memory in N dispatches.",
        content: "Goal-host-vessel currently performs compute_state_signature, deep-clones " +
        ...
        check_hints: [
            {
                target_vessel: "goal-host-vessel",
                forbidden_pattern_regex: "compute_state_signature.*per[-_]?dispatch|snapshotProxyBus|deep[-_]?clone.*bus",
                detail: "Look for unconditional state-capture in the dispatch hot path; " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "structural",
        summary: "Activities should be dispatchable; selection logic baked into vessel code violates the activity-as-primitive principle.",
        content: "Per IAL: 'Activities Constrain Search' applies recursively — the " +
        ...
        check_hints: [
            {
                forbidden_pattern_regex: "selectBestTemplate|chooseTemplate|recommendTemplate\\(|rankTemplates",
                detail: "In-vessel template selection functions should be dispatched as " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "guidance",
        summary: "Architectural insights should be queryable as concepts, not encoded as runtime checks.",
        content: "Encoding architectural rules as if-statements inside resolver source " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "structural",
        summary: "Resolvers live where data lives. Vessels own resolution for their own shapes; backend only stores traces.",
        content: "From IMPULSE_ACTIVITY_FOUNDATION.md design principle #3. Anti-pattern: " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "guidance",
        summary: "Every execution is traced. Activities that don't record traces violate the learning-loop foundation.",
        content: "IAL principle: trace storage is the raw material for Thompson Sampling " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "structural",
        summary: "No silent failures. Component failures must emit substrate-observable signals " +
            "(substrateGap, structured error, or shape-typed impulse) — not be dropped silently.",
        content: "[principle__no_silent_failures] Component failures must emit substrate-observable " +
    ...
    },
    {
        ...
        shape: "architectural_pattern_principle",
        severity: "structural",
        summary: "Substrate detection coverage must grow by detecting failure traces orthogonal " +
            "to all current architectural_pattern_principle concepts and authoring new ones.",
        content: "[principle__novel_failure_modes_via_orthogonality] The substrate's detector " +
...
const INDEX_CONCEPT = {
    ...
    severity: "advisory",
    summary: "Index of architectural principles by detector — self-documenting catalogue for the four horizon detectors.",
...
async function principleExists(apiKey, signature) {
    ...
    try {
        ...
        for (const c of list) {
            if ((typeof c.summary === "string" && c.summary.includes(signature)) ||
...
async function mintPrinciple(apiKey, entry) {
...
async function main() {
    ...
    const all = [...HAND_LISTED_PRINCIPLES, INDEX_CONCEPT];
...
//# sourceMappingURL=seed-architectural-principles.js.map