...
function globSync(pattern, opts) {
    return Array.from(new bun_1.Glob(pattern).scanSync({ cwd: opts.cwd }));
}
...
function checkCite(filePath, startLine, endLine, nearbyText) {
    const absPath = (0, node_path_1.join)(REPO_ROOT, filePath);
    if (!(0, node_fs_1.existsSync)(absPath)) {
        return { pass: false, reason: `file not found: ${filePath}` };
    }
    const lines = (0, node_fs_1.readFileSync)(absPath, "utf-8").split("\n");
...
function extractCites(markdownPath) {
    const content = (0, node_fs_1.readFileSync)(markdownPath, "utf-8");
    const results = [];
    let m;
...
async function main() {
    ...
    if (args.length === 0) {
        ...
        args.push(...files.slice(-10).map(f => (0, node_path_1.join)(REPO_ROOT, f)));
    }
    const targets = [];
    for (const arg of args) {
        if (arg.includes("*")) {
            targets.push(...globSync(arg, { cwd: REPO_ROOT }).map(f => (0, node_path_1.join)(REPO_ROOT, f)));
        }
        else {
            targets.push(arg.startsWith("/") ? arg : (0, node_path_1.join)(REPO_ROOT, arg));
        }
    }
    let totalCites = 0;
    let fails = 0;
...
//# sourceMappingURL=cite-check.js.map