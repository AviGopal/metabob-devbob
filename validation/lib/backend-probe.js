...
exports.snapshotRelevanceBefore = snapshotRelevanceBefore;
exports.runBackendProbe = runBackendProbe;
exports.renderBackendSection = renderBackendSection;
...
async function snapshotRelevanceBefore(endpoint, apiKey) {
    return fetchRelevanceSnapshot(endpoint, apiKey, "before");
}
async function runBackendProbe(opts) {
    const errors = [];
    ...
    let relevanceBefore = opts.relevanceBefore ?? emptySnapshot();
    ...
    if (executionIdsFound.length > 0) {
        try {
            const fetched = await fetchTracesForRun(opts.metabobEndpoint, opts.metabobApiKey, opts.runStartTime);
    ...
    const roots = dedupedTraces.filter((t) => !t.parent_execution_id || !byId.has(t.parent_execution_id));
    ...
    for (const t of dedupedTraces) {
        ...
        const isLifecycle = LIFECYCLE_ACTIVITIES.has(aid) ||
    ...
    for (const t of dedupedTraces) {
        const aid = t.activity_id ?? "";
        const tier = aid.includes("slot-binding") || aid.includes("validator-dispatch") || aid.includes("ribosome")
            ? "deterministic"
            : t.task_count > 0
                ? "llm"
                : "unknown";
    ...
    const vesselSet = new Set(dedupedTraces.map((t) => t.vessel_id).filter(Boolean));
    ...
    const vesselTraceCounts = new Map();
    for (const t of dedupedTraces) {
        if (t.vessel_id)
            vesselTraceCounts.set(t.vessel_id, (vesselTraceCounts.get(t.vessel_id) ?? 0) + 1);
    }
    ...
    let relevanceAfter = emptySnapshot();
    try {
        relevanceAfter = await fetchRelevanceSnapshot(opts.metabobEndpoint, opts.metabobApiKey, "after", opts.runStartTime);
    }
    catch (e) {
...
async function extractDiscoveryLogResolutions(stderrLogPath) {
    if (!(0, node_fs_1.existsSync)(stderrLogPath))
        return [];
    const text = await (0, promises_1.readFile)(stderrLogPath, "utf8");
    const results = [];
    // Match: [Impulse] Resolved via vessel discovery: <type> (shape: <shape>, vessel: <name>)
    for (const m of text.matchAll(/\[Impulse\] Resolved via vessel discovery: (\S+) \(shape: (\S+), vessel: ([^)]+)\)/g)) {
        results.push({ shape: m[2] ?? m[1] ?? "unknown", vessel: m[3] ?? "unknown" });
    }
    return results;
}
async function extractExecutionIds(stdoutLogPath) {
    if (!(0, node_fs_1.existsSync)(stdoutLogPath))
        return [];
    const text = await (0, promises_1.readFile)(stdoutLogPath, "utf8");
    const ids = new Set();
...
async function fetchTraceDetail(endpoint, apiKey, executionId) {
...
async function fetchTracesForRun(endpoint, apiKey, since) {
    ...
    if (!resp.ok)
        throw new Error(`GET execution-traces: ${resp.status} ${resp.statusText}`);
    const data = (await resp.json());
...
async function fetchRelevanceSnapshot(endpoint, apiKey, label, since) {
    ...
    while (offset < 5000) {
        ...
        if (!resp.ok)
            throw new Error(`GET impulse-relevance (${label}): ${resp.status}`);
        const data = (await resp.json());
...
function renderBackendSection(probe) {
    const lines = [];
...
//# sourceMappingURL=backend-probe.js.map