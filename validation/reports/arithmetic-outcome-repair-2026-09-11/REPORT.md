...
hour, so the qualification family (credit 10/11, budget 0.2, due threshold
1.0) comes due roughly every 5.3 hours — the multi-hour quiet between firings
is the scheduler's design, not a stall; a layer-wide `enqueued=0` tick is
legible idle, not absence.
