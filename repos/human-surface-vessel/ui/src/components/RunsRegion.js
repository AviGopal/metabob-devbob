...
function position(rows, fallback) {
    return rows.map((row) => ({
        row,
...
export function RunsRegion({ selectedDispatchId, onSelect, }) {
    ...
    useEffect(() => {
        ...
        setCommitted((prev) => {
            ...
            const refreshed = prev
                ...
                .filter((p) => incomingById.has(p.dispatchId) ||
        ...
        for (const p of arrivals)
            knownRef.current.add(p.dispatchId);
    ...
    const onListKeyDown = useCallback((event) => {
        ...
        if (event.key === "ArrowDown")
            next = Math.min(from + 1, committed.length - 1);
        if (event.key === "ArrowUp")
            next = Math.max(from - 1, 0);
        if (event.key === "Home")
            next = 0;
        if (event.key === "End")
            next = committed.length - 1;
        const target = committed[next];
        if (!target)
            return;
        event.preventDefault();
        setFocusedId(target.dispatchId);
        const el = scrollRef.current?.querySelector(`[data-dispatch-id="${CSS.escape(target.dispatchId)}"]`);
        el?.focus();
    ...
    const errorMessage = board.error instanceof Error ? board.error.message : "the upstream did not answer";
    const nonTerminalSeen = new Set();
    for (const p of committed) {
        if (p.row.status === "running" && nonTerminalSeen.size < LIVE_DETAIL_BUDGET) {
            nonTerminalSeen.add(p.dispatchId);
        }
    }
    return (_jsxs("section", { className: "sf-region", "aria-labelledby": "sf-runs-title", ...handlers, children: [_jsxs("div", { className: "sf-region-head", children: [_jsx("h2", { className: "sf-region-title", id: "sf-runs-title", children: "Runs" }), _jsx(LiveControls, { frozen: frozen, regionName: "runs" })] }), _jsxs("div", { className: "sf-runs-viewport", children: [buffered.length > 0 ? (_jsxs("button", { type: "button", className: "sf-new-pill", onClick: acceptBuffered, children: ["\u2191 ", buffered.length, " new \u2014 show ", buffered.length === 1 ? "it" : "them"] })) : null, _jsxs("div", { className: "sf-runs-scroll", ref: scrollRef, role: "log", "aria-live": "polite", "aria-relevant": "additions text", "aria-busy": board.isFetching, "aria-label": "Dispatched runs, newest first", onKeyDown: onListKeyDown, children: [failed && committed.length === 0 ? (_jsxs("p", { className: "sf-error", role: "status", style: { margin: "var(--sf-space-4)" }, children: ["The board could not be read: ", errorMessage, ". This is the surface failing, not the fleet being idle \u2014 nothing here says anything about what is running."] })) : null, committed.length === 0 && !failed ? (_jsx("p", { className: "sf-empty", children: board.isLoading ? ("Reading the board…") : (_jsxs(_Fragment, { children: [_jsx("strong", { children: "Nothing has been dispatched yet." }), "Ask for something in the box above and it lands here, verdict first \u2014 reached or not reached, never a status."] })) })) : null, committed.map((p) => (_jsx(RunRow, { row: p.row, startedAtMs: p.startedAtMs, now: now, selected: p.dispatchId === selectedDispatchId, liveDetail: nonTerminalSeen.has(p.dispatchId), onSelect: onSelect, intervalMs: intervalMs, tabStop: p.dispatchId === rovingId, onFocused: setFocusedId }, p.dispatchId)))] })] }), _jsx("div", { className: "sf-region-head", style: { borderTop: "1px solid var(--sf-rule)", borderBottom: "none" }, children: _jsx("p", { className: "sf-note", style: { margin: 0 }, children: "The board carries at most 50 runs. A verdict here is what the walk recorded \u2014 open a run to see what it actually produced." }) })] }));
}
//# sourceMappingURL=RunsRegion.js.map