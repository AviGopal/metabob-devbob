...
import { answerSolicitation, dispatchGoal, fetchActiveDispatches, fetchCapability, fetchFleetShapes, fetchRenderPolicy, fetchWalkState, injectContext, submitGrade, } from "./client";
export const queryKeys = {
    board: ["activeDispatches"],
    walk: (dispatchId) => ["goalWalkState", dispatchId],
    shapes: ["fleetShapes"],
    capability: (shape) => ["vesselCapability", shape],
...
export function useWalk(dispatchId, opts) {
    return useQuery({
        queryKey: queryKeys.walk(dispatchId ?? ""),
        queryFn: () => fetchWalkState(dispatchId),
 ...
 */
export function useFleetShapes() {
    return useQuery({
...
export function useCapability(shape, enabled) {
    return useQuery({
