 ...
 */
async function trySurfaceIntent(instruction) {
    let res;
    try {
    ...
    if (res.status === 422)
        return null; // not about the surface — send it to the walk
    if (!res.ok)
        return null;
...
export async function dispatchGoal(req) {
...
/* ────────────────────────────── resolve ──────────────────────────────────── */
async function resolveShape(payload) {
...
export function fetchWalkState(dispatchId) {
    return resolveShape({ type: "goalWalkState", dispatchId });
}
export async function fetchActiveDispatches() {
    const body = await resolveShape({
 ...
 */
export async function fetchFleetShapes() {
    const res = await fetch("/api/discovery/shapes", { credentials: "same-origin" });
    if (!res.ok)
        throw new SurfaceError(`shape list unavailable (HTTP ${res.status})`, res.status);
    const body = await readJson(res);
    return Array.isArray(body.shapes) ? body.shapes.filter((s) => typeof s === "string") : [];
}
/**
 ...
 */
export async function fetchCapability(shape) {
 ...
 */
export async function submitGrade(g) {
    ...
    if (!res.ok) {
        const body = await readJson(res);
        throw new SurfaceError(str(body.error) ?? `the verdict was not recorded (HTTP ${res.status})`, res.status);
    }
}
/**
 ...
 */
export async function fetchRenderPolicy() {
    const res = await fetch("/api/render-policy", { credentials: "same-origin" });
    if (!res.ok)
        throw new Error(`render policy unavailable (${res.status})`);
