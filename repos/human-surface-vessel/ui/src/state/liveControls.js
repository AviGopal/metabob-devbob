...
const LiveControlsContext = createContext(null);
export function LiveControlsProvider({ children }) {
    const [paused, setPaused] = useState(false);
    const [intervalMs, setIntervalMs] = useState(DEFAULT_INTERVAL_MS);
    const value = useMemo(() => ({ paused, setPaused, intervalMs, setIntervalMs }), [paused, intervalMs]);
    return _jsx(LiveControlsContext.Provider, { value: value, children: children });
}
export function useLiveControls() {
    const ctx = useContext(LiveControlsContext);
    if (!ctx)
        throw new Error("useLiveControls must be used inside LiveControlsProvider");
    return ctx;
}
/**
 ...
 */
export function useRegionFreeze() {
