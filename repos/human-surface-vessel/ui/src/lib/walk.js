 ...
 */
export function walkLogText(entry) {
 ...
 */
export function progressFingerprint(walk) {
    return [
        walk.status,
        String(walk.reached),
...
export function hasProgress(walk) {
    return (walk.walkLog.length > 0 ||
        walk.steps.length > 0 ||
...
export function detectSolicitation(walk) {
    ...
    for (const line of lines) {
        if (!SOLICITATION_MARKER.test(line))
            continue;
 ...
 */
export function pathExplanation(walk) {
    if (!walk.executionPath)
        return null;
    const attempts = walk.attemptCount;
    const template = walk.selectedTemplateId;
    const parts = [];
    if (template)
        parts.push(`it chose template ${template}`);
    if (typeof attempts === "number" && attempts > 1)
        parts.push(`after ${attempts} attempts`);
    if (walk.grounded === false)
        parts.push("and it was not grounded in anything it read");
    if (walk.grounded === true)
        parts.push("grounded in what it read");
