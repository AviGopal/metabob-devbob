 ...
 *    branch anywhere.
 */
import { CONTENT_FORMS, DEFAULT_CONTENT_FORM, NON_PINNABLE_FORMS, } from "@avigopal/design-tokens";
 ...
 */
export function heuristicForm(shape, text, fromEnvelope) {
    if (DIFF_HEADER.test(text))
        return "diff";
    if (DIFF_HINT.test(shape) && /^[+-]/m.test(text))
        return "diff";
    ...
    if (looksLikeRows(text))
        return "rows";
    if (ROWS_HINT.test(shape) && looksLikeRows(text))
        return "rows";
    ...
    if (fromEnvelope) {
        ...
        if (lines.length >= 2)
            return "terminal";
        if (lines.length === 1 && trimmed.length <= SCALAR_MAX_CHARS)
            return "scalar";
    }
    if (looksLikeProse(text))
        return "prose";
    if (PROSE_HINT.test(shape) && trimmed.split(/\s+/).length > 12)
        return "prose";
...
function isCommandEnvelope(keys) {
    return keys.every((k) => COMMAND_ENVELOPE_KEYS.has(k));
}
function isPinnable(form) {
    return !NON_PINNABLE_FORMS.includes(form);
}
function envelopeMeta(o, shape, payload) {
    const chips = [];
    const code = o["exit_code"];
    if (typeof code === "number" && Number.isFinite(code))
        chips.push({ kind: "exit", code });
 ...
 */
const TRUNCATED_ENVELOPE = /^\s*\{"shape"\s*:\s*"([^"\\]{1,80})"\s*,\s*(?:"[A-Za-z_][A-Za-z0-9_]*"\s*:\s*(?:"[^"\\]*"|-?\d+(?:\.\d+)?|true|false|null)\s*,\s*)*"(stdout|stderr|content|text|output)"\s*:\s*"/;
export function truncatedEnvelopePayload(preview) {
...
export function planContent(shape, preview, truncated, formByShape) {
    if (preview.trim().length === 0)
        return { form: "empty", text: "" };
    ...
    if ((stdout !== null || stderr !== null) && isCommandEnvelope(keys)) {
        ...
        if (payload.trim().length === 0)
            return { form: "empty", text: "", meta, ...declared };
 ...
 * way round.
 */
export function detectForm(shape, text, override) {
    return planContent(shape, text, false, override).form;
}
export function isKnownForm(form) {
    return CONTENT_FORMS.includes(form);
}
...
function cell(v) {
    ...
    return { text: `{${Object.keys(v).length} fields}`, title, summarised: true };
}
export function parseRows(text) {
    ...
    if (trimmed.startsWith("[")) {
        try {
            ...
            const rows = parsed.map((row) => columns.map((c) => cell(row[c])));
            return { columns, rows };
        }
        catch {
            return null;
    ...
    const rows = lines.slice(1).map((l) => {
        const parts = l.split("\t");
        return columns.map((_, i) => ({ text: (parts[i] ?? "").trim() }));
    });
    return { columns, rows };
}
