 ...
 * true regardless.
 */
const FILESYSTEM_QUESTION = /\b(file|files|directory|directories|folder|repo|repository|tree|path|line count|lines of code|loc|checkout|worktree|clone|commit|branch)\b/i;
const COUNT_ANSWER = /^\s*[\d,]+\s*$/;
const PATH_LITERAL = /(\/[\w.@-]+){2,}/g;
export function describesFilesystem(goal, answerBody) {
    if (goal && FILESYSTEM_QUESTION.test(goal))
        return true;
    if (answerBody && COUNT_ANSWER.test(answerBody) && goal && /\b(how many|count|number of)\b/i.test(goal))
        return true;
    if (answerBody && PATH_LITERAL.test(answerBody))
        return true;
    return false;
}
export function extractPaths(sources) {
    const found = new Set();
