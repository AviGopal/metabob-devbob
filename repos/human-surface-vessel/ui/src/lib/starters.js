 ...
 * system will reject.
 */
const NOT_A_GOAL_TARGET = /(_write$|_delete$|_update$|_deprecate$|^obsidian:|^fs_|^goal$|^dispatch_id$|^activity_execution$|^goal_execution$|heartbeat|^human_input$|_result$|^parameter|^task_|^error$)/i;
/** The vocabulary is ragged: whole prose sentences have been registered as shape names. */
function isWellFormedShapeName(shape) {
    if (shape.length === 0 || shape.length > 44)
        return false;
    if (/\s/.test(shape))
        return false;
    if (!/^[A-Za-z][A-Za-z0-9_:.-]*$/.test(shape))
        return false;
    return true;
}
export function humanizeShape(shape) {
    return shape
        .replace(/^[a-z]+:/, "")
        .replace(/[_-]+/g, " ")
 ...
 */
const PLUMBING = /\b(interactor|federation|probe|llm|quota|render policy|surface intent|ui feedback|ui panel|ui question|bootstrap|liveness|handshake|registry|discovery|manifest|env|schema)\b/;
/**
...
const FAMILIES = [
    ...
    },
    {
        ...
        label: (h) => h,
        text: (h) => `Produce a ${h} for the substrate as it stands right now, and show me the evidence each conclusion rests on.`,
    },
    {
...
export function isHumanGoalTarget(shape) {
    if (!isWellFormedShapeName(shape))
        return false;
    if (NOT_A_GOAL_TARGET.test(shape))
        return false;
    if (PLUMBING.test(humanizeShape(shape)))
        return false;
    return true;
}
export function deriveStarters(shapes, limit) {
    ...
    for (const shape of eligible) {
        ...
        if (held === undefined || shape < held)
            byHuman.set(human, shape);
    }
 ...
 */
export function inferTargetShapes(goal, shapes, limit = 3) {
    const tokens = goal
        .toLowerCase()
        .split(/[^a-z0-9]+/)
        .filter((t) => t.length > 3);
    ...
    const scored = shapes
        ...
        for (const token of tokens) {
            for (const word of words) {
                if (word === token)
                    score += 3;
                else if (word.length > 3 && (word.startsWith(token) || token.startsWith(word)))
                    score += 1;
            }
        }
        return { shape, score };
