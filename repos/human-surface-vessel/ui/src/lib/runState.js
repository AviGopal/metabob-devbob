 ...
 */
import { isTerminal } from "@avigopal/design-tokens";
/**
...
export function deriveRunState(f) {
    ...
    if (f.awaitingAnswer)
        return "waiting";
    if (f.quietForMs !== null && f.quietForMs > STALL_AFTER_MS)
        return "stalled";
...
export function verdictSentence(args) {
    ...
    if (args.state === "waiting")
        return "Blocked on a human answer. Nothing advances until the question is answered.";
...
export function whatHappensNext(state, canInject) {
    switch (state) {
        case "reached":
...
export function stateIsTerminal(state) {
