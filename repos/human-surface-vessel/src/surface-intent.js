...
function currentPx(token, defaultPx, overrides) {
...
function scaleTokens(tokens, overrides, factor, minPx, maxPx, baseToken) {
    const detail = {};
    let clamped = false;
    let fromBase = 0;
    let toBase = 0;
...
function clauses(text) {
    return text
        ...
        .map((c) => c
        .replace(/^(?:please\s+|can you\s+|could you\s+|i(?:'d| would)? (?:like|want)(?: you)?(?: to)?\s+|you should\s+|just\s+)+/i, "")
        .trim())
 ...
 */
export function readSurfaceIntent(text, current) {
    const changes = [];
    const unparsed = [];
    // MERGE, never replace — start from what is already in force.
    const tokenOverrides = { ...current.tokenOverrides };
    const formByShape = { ...current.formByShape };
    let maxPreviewChars = current.maxPreviewChars;
    let ledgerDefaultExpanded = current.ledgerDefaultExpanded;
    ...
    for (const clause of clauses(text)) {
        ...
        if (/\b(?:reset|undo|revert|restore|clear)\b/.test(c) &&
            (/\b(?:default|defaults|normal|original|everything|all|it)\b/.test(c) ||
                /^(?:reset|undo|revert)$/.test(c.trim()))) {
            for (const k of Object.keys(tokenOverrides))
                delete tokenOverrides[k];
            for (const k of Object.keys(formByShape))
                delete formByShape[k];
        ...
        /* 2. drop ONE form pin. */
        const unpin = /\b(?:stop overriding|unpin|un-pin|stop pinning|reset|clear)\s+(?:the\s+)?([a-z_][a-z0-9_:.\-]*)/i.exec(clause);
        ...
        const formMatch = /\b(?:show|render|display|format|present|draw|lay ?out)\s+(?:the\s+)?([a-z_][a-z0-9_:.\-]*)\s*(?:impulses?|results?|outputs?|entries)?\s+(?:as|in|like|using)\s+(.+)$/i.exec(clause);
        if (formMatch && formMatch[1] !== undefined && formMatch[2] !== undefined) {
            ...
            if (NON_PINNABLE_FORMS.includes(form)) {
                unparsed.push({
                    text: clause,
        ...
        if (sizeMatch &&
            ...
            const res = scaleTokens(TEXT_TOKENS, tokenOverrides, wanted / base, TEXT_MIN_PX, TEXT_MAX_PX, "--sf-text-base");
            touchedTokens = true;
            changes.push({
        ...
        if ((wantsBigger || wantsSmaller) && !aboutSpacing) {
            ...
            const res = scaleTokens(TEXT_TOKENS, tokenOverrides, stepFactor(c, wantsBigger), TEXT_MIN_PX, TEXT_MAX_PX, "--sf-text-base");
            touchedTokens = true;
            changes.push({
                field: "tokenOverrides (type scale)",
                from: `${px(res.fromBase)} base`,
        ...
        const looser = /\b(?:looser|loose|roomier|airier|spacious|breathing room|more space|more spacing|relaxed|open it up)\b/.test(c);
        if (denser !== looser) {
            const res = scaleTokens(SPACE_TOKENS, tokenOverrides, denser ? 1 / 1.25 : 1.25, SPACE_MIN_PX, SPACE_MAX_PX, "--sf-space-3");
            touchedTokens = true;
            changes.push({
        ...
        /* 8. preview length. */
        if (previewish) {
            const noCap = /\b(?:no|without|remove|drop|stop|never)\b/.test(c) || /\b(?:in full|full text|everything)\b/.test(c);
...
//# sourceMappingURL=surface-intent.js.map