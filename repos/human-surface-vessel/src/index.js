...
import { DISCOVERY_SHAPES, HOST, PORT, VESSEL_ID, VESSEL_NAME, config, } from "./config.js";
import { deregisterFromDiscovery, discoveryStatus, startDiscoveryRegistration, } from "./discovery-registration.js";
import { impulsesRouter } from "./routes/impulses.js";
import { corsHeaders, proxyRouter } from "./routes/proxy.js";
import { counts, listPanels, recentAsserts, recentAttachments, recentEvents, recentFeedback, recentObservations, signatureInputs, subscribe, } from "./store.js";
const app = new Hono();
// ─── Liveness ───────────────────────────────────────────────────────────────
/**
...
function uiNotBuilt() {
    return new Response(JSON.stringify({
        error: "ui_not_built",
        message: "The human-surface-vessel UI has not been built. Expected a bundle at " +
...
async function serveUiFile(pathname) {
...
console.log(`[human-surface-vessel] listening on ${HOST}:${PORT} | discovery=${config.DISCOVERY_ENDPOINT} | shapes=${DISCOVERY_SHAPES.length}`);
