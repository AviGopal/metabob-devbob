...
async function main() {
    ...
    for (let i = 0; i < BATCH; i++) {
        if (Date.now() - startedAt > DEADLINE_MS) {
            console.log(JSON.stringify({ drained: i, stop: "deadline" }));
            return;
        }
        let b;
        try {
        ...
        if (!b)
            return; // transport failure: reported by composeOne
        const gapId = b["gap_id"] ?? null;
        if (!gapId) {
            console.log(JSON.stringify({ drained: i, stop: "no_open_gap" }));
            return;
        }
        ...
        if (seen.has(gapId)) {
            console.log(JSON.stringify({ drained: i, stop: "repeat_gap", gap_id: gapId }));
            return;
        }
        seen.add(gapId);
        const c = (b["compose"] ?? {});
...
await main();
//# sourceMappingURL=gap-compose-tick.js.map