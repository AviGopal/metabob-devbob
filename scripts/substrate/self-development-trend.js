...
if (!ENABLED) {
    console.log("[self-dev-trend] disabled");
    process.exit(0);
}
// Read SurrealDB creds from the substrate env (same pattern as spectral-gap.ts).
let NS = "activity-system", DB = "learning_loop", URL = "http://localhost:8000", USER = "root", PASS = "root";
try {
    ...
    for (const line of env.split("\n")) {
        ...
        if (k === "SURREAL_URL")
            URL = v;
        else if (k === "SURREAL_USER" || k === "SURREALDB_USERNAME")
            USER = v;
        else if (k === "SURREAL_PASS" || k === "SURREALDB_PASSWORD")
            PASS = v;
        else if (k === "SURREAL_NS")
            NS = v;
        else if (k === "SURREAL_DB")
            DB = v;
    }
}
...
//# sourceMappingURL=self-development-trend.js.map