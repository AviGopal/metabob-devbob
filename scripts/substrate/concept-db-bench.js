...
const resolve = (pointer, timeoutMs) => http("POST", "/v2/impulses/resolve", { pointer }, timeoutMs);
...
const phases = {
    ...
    async vitals() {
        ...
        return {
            ...
            upkeep: {
                ...
                arms: (upkeep.json?.activity_summary ?? []).map((a) => ({ id: a.id, ev: a.expectedValue, trials: a.totalTrials })),
            },
        };
    },
    ...
    async seed(args) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (r.status >= 200 && r.status < 300 && !r.json?.error) {
                ok++;
                lat.push(r.ms);
            }
            else
    ...
    async "bench-graph"() {
        ...
        const ids = (list.json?.concepts ?? []).map((c) => String(c.id).replace(/^concept:/, ""));
        if (ids.length < 5)
            return { error: "need >=5 bench concepts; run seed first" };
    ...
    async "loop-probe"() {
        ...
        const ids = (list.json?.concepts ?? []).map((c) => String(c.id).replace(/^concept:/, ""));
        if (!ids.length)
            return { error: "no bench concepts; run seed first" };
        const id = ids[0];
...
async function main() {
    ...
    const run = async (name, a = []) => {
        ...
        console.log(line);
        if (OUT)
            await Bun.write(OUT, ((await Bun.file(OUT).exists()) ? await Bun.file(OUT).text() : "") + JSON.stringify(record) + "\n");
    };
    if (!cmd || cmd === "all") {
        for (const p of ["vitals", "shapes", "bench-embed", "bench-search", "bench-graph"])
            await run(p);
    }
    else {
        await run(cmd, args);
...
main();
//# sourceMappingURL=concept-db-bench.js.map