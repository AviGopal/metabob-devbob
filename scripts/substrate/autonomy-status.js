...
function hostRemap(ep) {
    const m = ep.match(/^(https?:\/\/)(localhost|127\.0\.0\.1):8(\d{3})(\/.*)?$/);
    return m ? `${m[1]}${m[2]}:18${m[3]}${m[4] ?? ""}` : null;
}
const shapeEndpointCache = new Map();
async function discoverEndpoints(shape) {
    ...
    for (const disc of DISCOVERY_CANDIDATES) {
        try {
            ...
            if (!vessels.length)
                break; // registry answered: no producer — fall back, don't retry other discovery eps
            const eps = [];
            ...
            if (eps.length) {
                shapeEndpointCache.set(shape, eps);
                return eps;
            }
        }
...
async function resolveShape(type, extra = {}, timeoutMs = 20000) {
...
async function fleetStats() {
    for (const ep of [process.env.DISCOVERY_ENDPOINT, "http://localhost:18100", "http://127.0.0.1:8100"].filter(Boolean)) {
        try {
...
async function readSubstrate(path) {
...
const sources = [];
if (process.env.METRICS_OUT)
    sources.push({ label: process.env.METRICS_OUT, text: await readHost(process.env.METRICS_OUT) });
...
for (const s of sources) {
    if (s.text && lastAt(s.text) > lastAt(best.text))
        best = s;
}
...
const dline = (label, body) => console.log(`    ${label.padEnd(16)} ${body}`);
...
//   gap-compose actually acts on (open/stale/churn), with no client-side capping.
{
    ...
    if (gl?.total_gaps != null) {
        ...
        if (staleCats)
            dline("", `stale top: ${staleCats} — should FALL without operator fixes`);
    }
    else {
...
// — self-alteration funnel (selfAlterationFunnelReport) — authored→staged→landed→pushed
{
    ...
    if (fu?.funnel) {
        ...
        if (fu.backlog_size != null)
            dline("", `proposal backlog ${fu.backlog_size} (${fu.stale_backlog} stale, ${((fu.stale_fraction ?? 0) * 100).toFixed(0)}%) — note: landed may include operator cutovers (see caveat field)`);
    }
    else {
...
const persistentlyNull = (path) => recent.every((r) => path.split(".").reduce((o, k) => (o == null ? o : o[k]), r) == null);
const dark = [];
if (persistentlyNull("gaps.model_reality_open"))
    dark.push("gaps");
if (persistentlyNull("dec_limiters.rho_sample_traces_per_hour"))
    dark.push("ρ_sample");
if (persistentlyNull("backward_model.composition_edges"))
    dark.push("λ₁/edges");
if (persistentlyNull("forward_model.selector_scored_fraction"))
    dark.push("selector");
...
if (dark.length)
    console.log(`  ⚠ BLIND PROBES   ${dark.join(", ")} reading null — restore before trusting green`);
const fmt = (label, cur, d, mark, note) => `  ${label.padEnd(16)} ${String(cur ?? "·").padStart(7)}   Δ${d.padStart(6)}  ${mark}  ${note}`;
console.log(fmt("self-alteration", last.self_alteration?.landed, delta("self_alteration.landed"), arrow("self_alteration.landed", "up"), "landed proposals [self_alteration_funnel_scan]"));
console.log(fmt("model-reality gaps", last.gaps?.model_reality_open, delta("gaps.model_reality_open"), arrow("gaps.model_reality_open", "down"), "open gaps — should fall without operator fixes [substrateGap / gap_lifecycle_scan]"));
console.log(fmt("backward model", last.backward_model?.composition_edges, delta("backward_model.composition_edges"), arrow("backward_model.composition_edges", "up"), "composition edges [learning_transfer_report.genuine_edge_density / compositionSuccess]"));
console.log(fmt("  └ orphan RECENT", last.backward_model?.recent_orphan_rate, delta("backward_model.recent_orphan_rate"), arrow("backward_model.recent_orphan_rate", "down"), `60m dangling-parent rate (${last.backward_model?.recent_composition_count ?? "·"} compositions) [trace_outcome_validity_audit]`));
console.log(fmt("learn-speed MGD", last.posterior_convergence?.managed_converged_frac, delta("posterior_convergence.managed_converged_frac"), arrow("posterior_convergence.managed_converged_frac", "up"), `converged ÷ Thompson-managed (${last.posterior_convergence?.converged ?? "·"}c/${last.posterior_convergence?.learning ?? "·"}l) [posterior_consistency_audit / variantMetricsSummary]`));
console.log(fmt("#6 uncertainty", last.posterior_uncertainty?.mean_variance, delta("posterior_uncertainty.mean_variance"), arrow("posterior_uncertainty.mean_variance", "down"), `mean Beta variance, ${last.posterior_uncertainty?.managed_cells ?? "·"} managed cells — DOWN = learning [variantMetricsSummary]`));
console.log(fmt("explore breadth", last.capability?.exploration_breadth, delta("capability.exploration_breadth"), arrow("capability.exploration_breadth", "up"), `distinct activities 24h ÷ total (${last.capability?.distinct_exercised_24h ?? "·"}/${last.capability?.total_activities ?? "·"}) [selectionEntropy / executionTraces]`));
console.log(fmt("cross-vessel comp", last.capability?.cross_vessel_frac, delta("capability.cross_vessel_frac"), arrow("capability.cross_vessel_frac", "up"), `edges spanning vessels (${last.capability?.cross_vessel_edges ?? "·"}/${last.capability?.total_edges ?? "·"}) [compositionSuccess]`));
console.log(fmt("shape closure", last.capability?.real_closure, delta("capability.real_closure"), arrow("capability.real_closure", "up"), `composed ÷ produced over REAL shapes (genuine orphans: ${last.capability?.orphan_genuine ?? "·"}) [composition_coverage_report]`));
console.log(fmt("#12 info (concepts)", last.substrate_self?.concepts, delta("substrate_self.concepts"), arrow("substrate_self.concepts", "up"), `concept-db size — accumulated information [conceptUsageStats]`));
console.log(fmt("#12 resource-eff", last.substrate_self?.llm_task_fraction, delta("substrate_self.llm_task_fraction"), arrow("substrate_self.llm_task_fraction", "down"), `LLM-task fraction (${last.substrate_self?.sampled_tasks ?? "·"} sampled) — LOW = efficient [resolverCostAnalysis]`));
...
const scored = terms.filter((t) => t.score != null);
...
const growth = (k, label, shape) => {
    const pts = series.filter((s) => s[k] != null);
    if (pts.length < 4) {
        console.log(`    ${label.padEnd(16)} (insufficient points)`);
        return;
    }
    ...
    if (overall == null) {
        console.log(`    ${label.padEnd(16)} (gaps)`);
        return;
    }
    let mark = "→ steady";
    if (s1 != null && s2 != null)
        mark = s2 > s1 + 0.05 ? "⤴ accelerating" : s2 < s1 - 0.05 ? "⤵ decelerating" : "→ steady";
...
// ── SPECTRAL-GAP GOVERNOR (λ₁ ≳ ρ_grow — gates capability minting) ───────────
{
    ...
    const genNow = (last.capability && last.capability.genuine_edges);
...
console.log("");
//# sourceMappingURL=autonomy-status.js.map