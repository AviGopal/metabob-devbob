...
async function fetchAdvertisedShapes() {
    try {
...
async function fetchDescribedShapes() {
    try {
 ...
 */
async function gatherEvidence(shape) {
    const parts = [];
    // (a) producing activities — the most reliable, human-authored signal.
    try {
        const esc = shape.replace(/"/g, '\\"');
        const [acts] = await sql(`SELECT name, description FROM activity WHERE output_shapes CONTAINS "${esc}" AND deprecated != true LIMIT 4;`);
        for (const a of acts || []) {
            const name = (a?.name ?? "").toString().slice(0, 80);
            const desc = (a?.description ?? "").toString().slice(0, 140);
            if (name || desc)
                parts.push(`- producer activity "${name}": ${desc}`);
        }
    }
    ...
    try {
        const esc = shape.replace(/"/g, '\\"');
        const [imps] = await sql(`SELECT pointer FROM impulse WHERE shape = "${esc}" ORDER BY created_at DESC LIMIT 1;`);
        const p = (imps && imps[0]) ? imps[0].pointer : null;
        if (p)
            parts.push(`- recent impulse pointer structure: ${JSON.stringify(p).slice(0, 200)}`);
    }
    catch { /* skip this evidence source */ }
    return parts.join("\n");
}
/** Ask the canonical LLM resolver for ONE concrete line. Returns null on failure. */
async function generateDescription(shape, evidence) {
    const prompt = `You are cataloguing a software substrate's data resolvers so an automated planner ` +
    ...
    try {
        ...
        if (!r.ok)
            return null;
        const d = (await r.json());
        if (!d.resolved || typeof d.content !== "string")
            return null;
...
async function postLearnedDescription(shape, description) {
    try {
...
async function main() {
    ...
    console.log(`[auto-describe] tick done: gap=${undescribed.length} batch=${batch.length} applied=${applied} ` +
...
//# sourceMappingURL=auto-describe-resolvers.js.map