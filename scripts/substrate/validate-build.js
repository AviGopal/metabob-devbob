...
function isTracked(dir, file) {
    try {
        const out = (0, node_child_process_1.execSync)(`git -C "${dir}" ls-files -- "${file}"`, {
            encoding: "utf8",
        ...
        return (0, node_fs_1.existsSync)((0, node_path_1.join)(dir, file));
    }
}
const REPO_ROOT = (0, node_path_1.resolve)(import.meta.dir, "../..");
const DOCKERFILE = (0, node_path_1.join)(REPO_ROOT, "Dockerfile.substrate");
const JSON_OUT = process.argv.includes("--json");
const findings = [];
const err = (vessel, msg) => findings.push({ vessel, level: "error", msg });
const warn = (vessel, msg) => findings.push({ vessel, level: "warn", msg });
const dockerfile = (0, node_fs_1.readFileSync)(DOCKERFILE, "utf8");
...
for (let i = 0; i < dfLines.length; i++) {
    ...
    for (let j = i + 1; j < dfLines.length && !/^COPY repos\//.test(dfLines[j]); j++) {
        if (/bun install/.test(dfLines[j]) && /frozen-lockfile/.test(dfLines[j]))
            frozen = true;
...
for (const [vessel, { frozen, seds }] of containerized) {
    const vDir = (0, node_path_1.join)(REPO_ROOT, "repos", vessel);
    const pkgPath = (0, node_path_1.join)(vDir, "package.json");
    if (!(0, node_fs_1.existsSync)(pkgPath)) {
        err(vessel, `package.json missing at repos/${vessel}/`);
        continue;
    }
    let pkg;
    try {
        ...
        err(vessel, `package.json is not valid JSON: ${String(e).split("\n")[0]}`);
        continue;
    }
    ...
    for (const [key, spec] of Object.entries(deps)) {
        ...
        const hostTarget = (0, node_path_1.resolve)(vDir, rel);
        if (!(0, node_fs_1.existsSync)((0, node_path_1.join)(hostTarget, "package.json"))) {
        ...
        if (rel.startsWith("../") && !sed) {
            ...
            const containerResolved = (0, node_path_1.resolve)(`/vessels/${vessel}`, rel);
        ...
        if (sed) {
            ...
            if (!to.startsWith("/vessels/"))
                warn(vessel, `sed rewrites "${rel}" to "${to}" which is not under /vessels`);
        }
    }
    ...
    if (frozen) {
        const lockPath = (0, node_path_1.join)(vDir, "bun.lock");
        ...
        if (!lockCommitted) {
            ...
            err(vessel, (0, node_fs_1.existsSync)(lockPath)
        ...
        else if ((0, node_fs_1.existsSync)(lockPath)) {
            const lock = (0, node_fs_1.readFileSync)(lockPath, "utf8");
    ...
    const srcDir = (0, node_path_1.join)(vDir, "src");
    if ((0, node_fs_1.existsSync)(srcDir)) {
        const files = [...new Bun.Glob("**/*.ts").scanSync({ cwd: srcDir })];
        const seen = new Set();
        for (const f of files) {
            const txt = (0, node_fs_1.readFileSync)((0, node_path_1.join)(srcDir, f), "utf8");
            for (const mm of txt.matchAll(importSpecRe)) {
                const spec = mm[1];
                if (!spec.startsWith("@"))
                    continue; // only scoped sibling-pkg imports
...
function git(dir, args) {
    try {
        return (0, node_child_process_1.execSync)(`git -C "${dir}" ${args}`, {
            encoding: "utf8",
            stdio: ["ignore", "pipe", "ignore"],
        }).trim();
    }
...
if (recordedPointers.size === 0) {
    ...
    for (const [vessel] of containerized) {
        ...
        if (!recorded || !head)
            continue; // vendored in-tree, not a submodule — nothing to compare
            ...
            const where = ahead > 0 && behind > 0
                ...
                : ahead > 0
                    ...
                    : behind > 0
                        ...
                        : "differs";
            const remedy = behind > 0 && ahead === 0
                ? `git submodule update --init repos/${vessel}`
                : `git add repos/${vessel} && git commit`;
            repro(vessel, `checkout ${head.slice(0, 8)} != super-repo pointer ${recorded.slice(0, 8)}: ${where}. ` +
        ...
        if (onRemote !== null && onRemote === "") {
            repro(vessel, `super-repo pins ${recorded.slice(0, 8)}, which is on NO remote-tracking branch — ` +
        ...
        if (porcelain) {
            ...
            if (untracked)
                parts.push(`${untracked} untracked path(s) — these DO enter the build context`);
...
let metabobNames = 0;
for (const [vessel] of containerized) {
    const pkg = JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.join)(REPO_ROOT, "repos", vessel, "package.json"), "utf8"));
    if (/metabob/.test(pkg.name ?? ""))
        metabobNames++;
}
if (JSON_OUT) {
    ...
    if (!findings.length)
        console.log("  ✓ all containerized vessels are build-valid (file: deps resolve, lockfiles synced, imports match deps)");
    console.log(`\n  migration: ${metabobNames} containerized vessel(s) still have a 'metabob' package name`);
    console.log(`\n${errors.length} error(s), ${warns.length} warning(s)`);
}
process.exit(findings.some((f) => f.level === "error") ? 1 : 0);
//# sourceMappingURL=validate-build.js.map