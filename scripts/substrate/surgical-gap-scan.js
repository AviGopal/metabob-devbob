...
const REPOS_ROOT = process.env.SURGICAL_SCAN_REPOS ?? (0, node_path_1.join)(import.meta.dir, "..", "..", "repos");
...
function discoverVessels() {
    try {
        return (0, node_fs_1.readdirSync)(REPOS_ROOT)
            .filter((v) => !v.startsWith(".") && (0, node_fs_1.existsSync)((0, node_path_1.join)(REPOS_ROOT, v, "src")))
            .sort();
    }
    catch {
        return [];
...
function findExistingConstForPort(content, port) {
    const re = new RegExp(`const\\s+(\\w+)\\s*=\\s*(?:process\\.env\\.\\w+\\s*(?:\\?\\?|\\|\\|)\\s*)?["'\`](?:https?|ws):\\/\\/(?:localhost|127\\.0\\.0\\.1):${port}\\b`, "m");
...
function walk(dir, cap = 4000) {
    ...
    while (stack.length && out.length < cap) {
        ...
        let entries;
        try {
            entries = (0, node_fs_1.readdirSync)(d);
        }
        catch {
            continue;
        }
        for (const e of entries) {
            if (e === "node_modules" || e === ".git" || e === "dist" || e.startsWith("."))
                continue;
            const p = (0, node_path_1.join)(d, e);
            let st;
            try {
...
function scanVessel(vessel) {
    const srcAbs = (0, node_path_1.join)(REPOS_ROOT, vessel, "src");
    if (!(0, node_fs_1.existsSync)(srcAbs))
        return [];
    const hits = [];
    for (const abs of walk(srcAbs)) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (/const\s+\w+\s*=\s*(["'`])https?:\/\//.test(text))
                continue;
            if (/\?\?/.test(text))
                continue;
            // repos/<vessel>/<relative under repo root>
            const rel = "repos/" + vessel + abs.slice((0, node_path_1.join)(REPOS_ROOT, vessel).length);
            hits.push({
                vessel,
...
function gapFromHit(h) {
    ...
    const hash = (0, node_crypto_1.createHash)("sha256").update(`${h.repoRel}|${h.line}|${h.port}|${h.url}`).digest("hex").slice(0, 10);
    ...
    const fixConst = h.existingConst
        ? `the env-defaulted endpoint constant \`${h.existingConst}\` ALREADY declared in this file (build the URL from it, e.g. \`\${${h.existingConst}}${(() => { try {
            return new URL(h.url).pathname;
        }
        catch {
            return "";
        } })()}\`) instead of the inline literal — a pure single-line substitution, no new declaration needed`
        : role
    ...
    const proposed_fix = `Replace the inline hardcoded endpoint literal \`${h.url}\` on line ${h.line} of ${h.repoRel} with ${fixConst}. ` +
    ...
    const summary = `Inline hardcoded endpoint literal "${h.url}" in ${h.repoRel}:${h.line}` +
...
await main();
//# sourceMappingURL=surgical-gap-scan.js.map