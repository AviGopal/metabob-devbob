...
function resolveApiKey() {
    try {
        const cfg = JSON.parse((0, node_fs_1.readFileSync)(`${process.env.HOME}/.metabob/config.json`, "utf8"));
        if (cfg?.metabob?.apiKey)
...
async function honestReach() {
    ...
    try {
        ...
        const labels = typeof res.content === "string" ? JSON.parse(res.content) : res.content ?? [];
        if (!labels.length)
            return console.log("no labels in corpus");
        const tally = (set) => {
...
async function selfReportedReach() {
    ...
    try {
        ...
        const dispatches = res?.body?.dispatches ?? [];
        if (!dispatches.length)
            return console.log("no dispatches in window");
        for (const [row, n] of countBy(dispatches, (d) => `${d.status}\treached=${d.reached}`))
...
async function gapTriple() {
    ...
    try {
        ...
        if (!gaps.length)
            return console.log("no gaps in store");
        ...
        if (expired > repaired)
            console.log("⚠ closes are expiry-dominated — the headline close rate overstates repair");
...
async function topology() {
    ...
    try {
        ...
        const vessels = res?.content?.vessels ?? [];
    ...
    try {
        ...
        if (proc.exitCode !== 0)
            throw new Error(proc.stderr.toString().trim() || `docker exec exit ${proc.exitCode}`);
    ...
    try {
        ...
        if (!m)
            throw new Error("template count not found in report");
        const n = Number(m[1]);
...
if (!API_KEY)
    console.log("⚠ no API key resolved (~/.metabob/config.json .metabob.apiKey or METABOB_API_KEY) — authed reads will fail");
await honestReach();
await selfReportedReach();
await gapTriple();
await topology();
//# sourceMappingURL=performance-status.js.map