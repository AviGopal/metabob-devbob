...
async function typecheckVessel(vessel) {
    const cwd = `${RUNTIME_DIR}/${vessel}`;
    if (!(await pathExists(`${cwd}/tsconfig.json`)))
        return [];
    ...
    for (const line of out.split("\n")) {
        ...
        if (m)
            errs.push({ vessel, file: m[1].trim(), line: +m[2], col: +m[3], code: m[4], message: m[5].trim() });
    }
    return errs;
}
async function main() {
    await (0, promises_1.mkdir)(SCENARIOS_DIR, { recursive: true });
    const existing = new Set(await (0, promises_1.readdir)(SCENARIOS_DIR).catch(() => []));
    const applied = new Set(await (0, promises_1.readdir)(APPLIED_DIR).catch(() => []));
    let gaps = [];
    try {
        const p = JSON.parse(await (0, promises_1.readFile)(GAPS_PATH, "utf-8"));
        if (Array.isArray(p))
            gaps = p;
    }
    catch { /* start fresh */ }
    ...
    for (const vessel of VESSELS) {
        if (created.length >= MAX_NEW)
            break;
        let errs = [];
        try {
            errs = await typecheckVessel(vessel.trim());
        }
        catch { /* tolerant */ }
        ...
        for (const e of errs) {
            ...
            if (existing.has(scenarioFile) || applied.has(reportSentinel))
                continue; // dedup
            ...
            const scenario = {
                ...
                description: `\`bun run typecheck\` (tsc --noEmit) fails in ${e.vessel} at ${e.file} ` +
            ...
            await (0, promises_1.writeFile)(`${SCENARIOS_DIR}/${scenarioFile}`, JSON.stringify(scenario, null, 2));
            if (!gapIds.has(id)) {
                gaps.push({
                    id,
    ...
    if (created.length > 0) {
        try {
            await (0, promises_1.writeFile)(GAPS_PATH, JSON.stringify(gaps, null, 2));
        }
        catch { /* tolerant */ }
    }
...
await main();
//# sourceMappingURL=typecheck-scenario-gen.js.map