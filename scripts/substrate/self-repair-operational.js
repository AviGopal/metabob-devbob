...
async function emitGap(gap) {
    try {
        ...
        console.warn("[self-repair] emit failed:", e.message);
    }
}
...
for (const t of inactive) {
    if (!REPAIR) {
        console.log(`[self-repair] DRY-RUN would enable --now ${t}`);
        continue;
    }
    ...
    if (r.ok && active) {
        repaired.push(t);
        console.log(`[self-repair] re-enabled ${t}`);
    }
    else {
        failed.push(t);
        console.warn(`[self-repair] FAILED to re-enable ${t}: ${r.out}`);
    }
}
...
//# sourceMappingURL=self-repair-operational.js.map