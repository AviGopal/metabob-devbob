...
import { createVesselLibp2p, serveResolveHttp } from '@avigopal/libp2p-federation-transport';
...
if (!RELAY || !DISCOVERY) {
    console.error('[obsidian-passthrough] set RELAY_MULTIADDR and DISCOVERY_URL');
    process.exit(1);
}
...
await serveResolveHttp(vl, async (pointer) => {
    ...
    try {
        ...
        return { error: 'obsidian plugin unreachable: ' + String(e?.message ?? e) };
    }
});
...
if (!circuit)
    console.warn('[obsidian-passthrough] no circuit multiaddr yet — registration will advertise none');
Bun.serve({
    port: HEALTH_PORT,
    fetch(req) {
...
async function register() {
    try {
        ...
        console.log('[obsidian-passthrough] register err', String(e));
    }
}
await register();
...
//# sourceMappingURL=obsidian-passthrough.js.map