...
const acts = (rows || []).filter((a) => a.id && !isHook(a.id));
const sigs = acts.map((a) => `${(a.name || a.id).slice(0, 80)} :: in:${(a.input_shapes || []).join(",")} out:${(a.output_shapes || []).join(",")}`);
...
const embeds = ((await er.json()).content || []).map((c) => c.embedding);
const norm = (v) => { let s = 0; for (const x of v)
    s += x * x; const n = Math.sqrt(s) || 1; return v.map((x) => x / n); };
...
for (let i = 0; i < E.length; i++)
    for (let j = i + 1; j < E.length; j++) {
        ...
        if (d > 0.9) {
            high++;
            redundant.push({ a: acts[i].id, b: acts[j].id, cosine: Math.round(d * 1e4) / 1e4 });
        }
    }
...
try {
    const f = "/workspace/metrics/coherence.jsonl";
...
try {
    await Bun.write("/workspace/metrics/coherence-candidates.json", JSON.stringify({ at: out.at, candidates: topRedundant }, null, 2));
}
catch { /* tolerant */ }
//# sourceMappingURL=coherence-metric.js.map