...
function gitRepos() {
    ...
    if ((0, node_fs_1.existsSync)((0, node_path_1.join)(superRepo, ".git")))
        out.push(superRepo);
    const vesselsParent = "/workspace/git/vessels";
    try {
        for (const name of (0, node_fs_1.readdirSync)(vesselsParent)) {
            const dir = (0, node_path_1.join)(vesselsParent, name);
...
async function emitGap(gap) {
    ...
    try {
        ...
        console.warn("[self-op-health] emit failed:", e.message);
    }
}
...
function flag(check, severity, detail, signature, extra = {}) {
...
for (const f of METRIC_FILES) {
    try {
        const ageMin = (now - (0, node_fs_1.statSync)(f).mtimeMs) / 60000;
        if (ageMin > FRESH_MAX_MIN) {
            flag("measurement_dark", "HIGH", `Measurement stream ${f.split("/").pop()} is ${ageMin.toFixed(0)}m stale (>${FRESH_MAX_MIN}m) → its collector timer is likely dead; autonomy is unobservable on this axis until restored.`, `dark:${f.split("/").pop()}`, { file: f, age_min: Math.round(ageMin) });
        }
    }
    catch {
        flag("measurement_missing", "HIGH", `Measurement stream ${f} does not exist → collector never ran in this deployment; KPI series cannot be built.`, `missing:${f.split("/").pop()}`, { file: f });
    }
}
...
const inactive = REQUIRED_TIMERS.filter((t) => {
    if (sh(["systemctl", "is-enabled", t]).out === "masked")
        return false;
    return sh(["systemctl", "is-active", t]).out !== "active";
});
if (inactive.length) {
    flag("self_correction_timer_inactive", "HIGH", `${inactive.length} required self-correction/measurement timer(s) inactive: ${inactive.join(", ")} → that self-correction loop is dead and the capability is silently lost until re-enabled (typical cause: runtime-only enablement not surviving a container recreate).`, `timers:${inactive.sort().join("+")}`, { inactive });
}
...
function inventoryUnits() {
    for (const f of ["/workspace/substrate/fleet/vessels.inventory.json", "/usr/local/share/substrate/vessels.inventory.json"]) {
        try {
            const j = JSON.parse((0, node_fs_1.readFileSync)(f, "utf8"));
            return new Set((j.vessels ?? []).map((v) => v.unit).filter((u) => !!u));
        }
...
const dormant = REPAIR_UNIT_CANDIDATES.filter((t) => {
    ...
    if (enabledState === "" || enabledState === "not-found" || enabledState === "masked")
        return false;
    return sh(["systemctl", "is-active", t]).out !== "active";
});
if (dormant.length) {
    const declared = dormant.filter((t) => invUnits.has(t));
    flag("repair_unit_dormant", "MEDIUM", `${dormant.length} built repair unit(s) loaded but disabled/inactive: ${dormant.join(", ")}${declared.length ? ` (inventory-declared: ${declared.join(", ")})` : " (none declared in the fleet inventory)"} → each is a repair mechanism that exists in the image yet never runs, with nothing else noticing. Not auto-enabled — dormancy needs a DISPOSITION: enable it (declare it in the fleet inventory / role selection) or retire the unit file; until then this repair capability is silently absent.`, `repair_dormant:${[...dormant].sort().join("+")}`, { dormant, inventory_declared: declared });
}
...
// follow reflect reality. Never merges divergence; failure is non-fatal.
{
    ...
    if (CLONES.includes(superRepo)) {
        ...
        if (f.ok)
            sh(["git", "-C", superRepo, "merge", "--ff-only", "origin/dev", "--quiet"], 10000);
    }
}
...
for (const dir of CLONES) {
    ...
    if (Number.isFinite(ct) && ct > newestSubstrateCommit)
        newestSubstrateCommit = ct;
}
if (newestSubstrateCommit === 0) {
    flag("self_commit_absent", "MEDIUM", `No substrate-authored commit found in any push clone (${CLONES.join(", ")}) → the self-development loop has never landed code from this deployment, or the clones are unwired.`, "self_commit:absent");
}
else {
    const ageHr = (now / 1000 - newestSubstrateCommit) / 3600;
    if (ageHr > COMMIT_MAX_HR) {
        flag("self_commit_stale", "MEDIUM", `Last substrate-authored commit was ${ageHr.toFixed(0)}h ago (>${COMMIT_MAX_HR}h) → the author→land→push loop is stalled; proposals may be drafting but not landing (check the cutover/funnel stage or the push credential).`, "self_commit:stale", { age_hr: Math.round(ageHr) });
    }
}
...
// but cannot push, which presents as check-3 staleness with this as root cause.
{
    ...
    if (!r.ok) {
        flag("push_credential_invalid", "HIGH", `git ls-remote origin failed in ${dir} → the push credential is invalid/expired or origin is unreachable; the substrate can author but cannot land changes. (Distinguish network vs credential at triage.)`, "push_cred:fail", { clone: dir });
    }
}
...
try {
    const lines = (0, node_fs_1.readFileSync)(AUTONOMY_METRICS, "utf8").trim().split("\n").filter(Boolean);
    const recs = [];
    ...
    if (recs.length >= 2) {
        ...
        if (dLanded <= 0 && dNested <= 0) {
            flag("progress_stall", "MEDIUM", `0 forward progress over ${PROGRESS_STALL_HR}h on BOTH axes — code-landing (self_alteration.landed Δ=${dLanded}) and topology (topology.nested Δ=${dNested}) — though measurement timers are live. Continuous operation is ALIVE but not ADVANCING: goals dispatch + the reach gate rejects them as hollow, so nothing reaches → nothing mints → no cutover lands → topology stays flat. Liveness checks 1-4 stay green through this; this makes the EFFICACY axis visible in S (the authoring-ceiling frontier).`, "progress:stall", { d_landed: dLanded, d_nested: dNested, window_hr: PROGRESS_STALL_HR, baseline_age_hr: Math.round((now - baseline.atMs) / 3600_000) });
        }
    }
    ...
    console.warn(`[self-operational-health] progress check skipped: ${e.message}`);
}
...
].map((d) => (0, node_path_1.join)(WORKSPACE_ROOT, d));
function writable(p) {
    try {
        (0, node_fs_1.accessSync)(p, node_fs_1.constants.W_OK);
        return true;
    }
    catch {
        return false;
...
if (!(0, node_fs_1.existsSync)(WORKSPACE_ROOT)) {
    flag("workspace_unreachable", "HIGH", `Workspace root ${WORKSPACE_ROOT} does not exist → the live volume is unmounted (typical: container-recreate race before the volume attaches). Effectively ALL goals that read/write the workspace (fs_* resolvers, proposals, patterns, cutover clones, metrics) fail with ENOENT until it is mounted. Operator-boundary: remounting is not something the substrate can safely self-repair.`, "workspace:root_missing", { root: WORKSPACE_ROOT });
}
else if (!writable(WORKSPACE_ROOT)) {
    flag("workspace_readonly", "HIGH", `Workspace root ${WORKSPACE_ROOT} exists but is NOT writable → fs_write goals, proposal/pattern emission, and cutover landing all fail with EACCES. Operator-boundary: a permission/remount fix is not in the self-repair allowlist.`, "workspace:root_ro", { root: WORKSPACE_ROOT });
}
else {
    const missing = REQUIRED_WORKSPACE_DIRS.filter((d) => !(0, node_fs_1.existsSync)(d));
    const readonly = REQUIRED_WORKSPACE_DIRS.filter((d) => (0, node_fs_1.existsSync)(d) && !writable(d));
    if (missing.length || readonly.length) {
        flag("workspace_subdir_unavailable", "MEDIUM", `${missing.length} required workspace subdir(s) missing${missing.length ? ` (${missing.map((d) => d.replace(WORKSPACE_ROOT + "/", "")).join(", ")})` : ""}` +
            `${readonly.length ? ` and ${readonly.length} not writable (${readonly.map((d) => d.replace(WORKSPACE_ROOT + "/", "")).join(", ")})` : ""} → goals that fs_write to them fail with ENOENT/EACCES until created (an fs_write that does not mkdir -p its parent is the usual trigger). Lower-severity than a root fault: only that class of goal fails.`, `workspace:subdirs:${[...missing, ...readonly].map((d) => d.replace(WORKSPACE_ROOT + "/", "")).sort().join("+")}`, { missing, readonly });
    }
}
...
const SPOOL_DIR = process.env.IAS_TRACE_SPOOL_DIR || (0, node_path_1.join)(WORKSPACE_ROOT, "trace-spool");
try {
    const spoolFiles = (0, node_fs_1.readdirSync)(SPOOL_DIR).filter((f) => f.endsWith(".json"));
    let oldestAgeMin = 0;
    let staleCount = 0;
    const staleSamples = [];
    let sampleEndpoint = "";
    for (const name of spoolFiles) {
        const file = (0, node_path_1.join)(SPOOL_DIR, name);
        try {
            const ageMin = (now - (0, node_fs_1.statSync)(file).mtimeMs) / 60000;
            if (ageMin <= SPOOL_MAX_MIN)
                continue;
            staleCount++;
            if (ageMin > oldestAgeMin)
                oldestAgeMin = ageMin;
            if (staleSamples.length < 5) {
                try {
                    const rec = JSON.parse((0, node_fs_1.readFileSync)(file, "utf8"));
                    staleSamples.push(rec.trace_id ?? name);
                    if (!sampleEndpoint && rec.endpoint)
                        sampleEndpoint = rec.endpoint;
                }
                catch {
                    staleSamples.push(name);
                }
            }
        }
...
//# sourceMappingURL=self-operational-health.js.map