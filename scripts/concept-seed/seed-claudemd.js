...
function parseSections(filePath, maxLevel) {
    const absolute = (0, node_path_1.resolve)(REPO_ROOT, filePath);
...
function shapeFromHeading(heading) {
    return heading
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_+|_+$/g, "")
...
async function createConcept(s, sourceType) {
...
async function linkConcepts(fromId, toId, edgeType, desc) {
...
async function main() {
    ...
    for (const source of SOURCES) {
        ...
        if (rootId)
            fileRoots.set(source.path, rootId);
    }
    ...
    if (foundationRoot) {
        ...
        for (const [path, rootId] of fileRoots) {
            if (path === "docs/architecture/IMPULSE_ACTIVITY_FOUNDATION.md")
                continue;
...
//# sourceMappingURL=seed-claudemd.js.map