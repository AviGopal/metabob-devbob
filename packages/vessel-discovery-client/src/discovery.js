 ...
 */
export async function discoverByShape(options) {
    const { shape, discoveryEndpoint, authToken, authType, cacheTtlMs = 60000, // 1 minute default
    logger, } = options;
    ...
    try {
        ...
        logger?.info(`[Discovery] Found ${result.vessels.length} vessel(s) for shape: ${shape}`);
...
export async function discoverVessels(options) {
    ...
    try {
        ...
        logger?.info(`[Discovery] Found ${response.content.totalCount} total vessel(s)`);
