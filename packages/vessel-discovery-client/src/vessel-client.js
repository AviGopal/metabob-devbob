...
export class VesselClient {
    ...
    constructor(config) {
        ...
        this.metrics = new VesselMetrics(this.config.metricsEmitter, this.logger);
    ...
    async register() {
        ...
        try {
            this.logger.info(`[VesselClient] Registering ${this.config.vesselId} at ${this.config.discoveryEndpoint}`);
            ...
            if (response.success) {
                this.logger.info(`[VesselClient] ✓ Registered successfully, expires at ${new Date(response.expiresAt).toISOString()}`);
    ...
    async heartbeat(metrics) {
        ...
        try {
            ...
            if (response.success) {
                ...
                this.logger.debug(`[VesselClient] ✓ Heartbeat successful (${latency}ms), next in ${response.nextHeartbeatMs}ms`);
                return true;
            }
            else {
                this.consecutiveFailureCount++;
                this.logger.warn(`[VesselClient] Heartbeat returned success=false (${this.consecutiveFailureCount}/${this.config.maxConsecutiveFailures})`);
                this.metrics.heartbeatFailure(this.config.vesselId, this.consecutiveFailureCount);
                return false;
            }
        }
        catch (error) {
            this.consecutiveFailureCount++;
            const errorMsg = error instanceof Error ? error.message : String(error);
            this.logger.error(`[VesselClient] Heartbeat failed (${this.consecutiveFailureCount}/${this.config.maxConsecutiveFailures}):`, errorMsg);
            this.metrics.heartbeatFailure(this.config.vesselId, this.consecutiveFailureCount);
            return false;
        }
    }
    ...
    startHeartbeat() {
        ...
        this.running = true;
        this.logger.info(`[VesselClient] Starting heartbeat (interval: ${this.config.heartbeatIntervalMs}ms)`);
        const runHeartbeat = async () => {
            ...
            if (!success &&
                ...
                this.logger.error(`[VesselClient] Max consecutive failures (${this.config.maxConsecutiveFailures}) reached, attempting re-registration`);
                this.stopHeartbeat();
                // Attempt re-registration with backoff
                const delay = this.backoff.nextDelay();
                this.logger.info(`[VesselClient] Re-registering in ${delay}ms (attempt ${this.backoff.getAttempts()})`);
                setTimeout(async () => {
                    ...
                    if (registered) {
                        ...
                        this.logger.error("[VesselClient] Max re-registration attempts reached, giving up");
                    }
                }, delay);
                return;
            }
...
//# sourceMappingURL=vessel-client.js.map