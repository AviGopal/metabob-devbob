 ...
 */
function extractAdvertisedShapes(source, filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inArray) {
            ...
            if (match) {
                ...
                if (depth <= 0)
                    inArray = false; // array opened and closed on one line
...
function extractDispatchCases(source) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = i - 1; j >= 0; j--) {
            ...
            if (prev === '')
                continue;
            if (prev.includes('@shape-dispatch:private')) {
                isPrivate = true;
                break;
            }
...
// Diff computation
// ---------------------------------------------------------------------------
function computeDiff(advertised, dispatched, configMappings, configFile, dispatchFile) {
    const findings = [];
    // Build a set of advertised shapes that are "covered" after applying mappings
    // mapping: advertisedShape → [caseLabel, ...]
    const advertisedResolved = new Map();
...
function main() {
    ...
    const mappings = configOverride.mappings ?? {};
    // Parse source files
    ...
    if (advertised.size === 0) {
        console.warn(`shape-dispatch-check: no advertised shapes found in ${configPath}.\n` +
            `  Expected a 'shapes: [...]' or 'advertised_shapes: [...]' array.`);
        // Not a hard error — vessel may not have discovery shapes yet.
        process.exit(0);
    }
    // Compute diff
    const findings = computeDiff(advertised, dispatched, mappings, configPath, dispatchPath);
    // Report
    ...
    console.error(`shape-dispatch-check: ${findings.length} violation(s) found in ${vesselRoot}\n`);
    ...
    console.error(`To map an advertised shape to different case label(s), create shape-dispatch.config.json with a "mappings" block.`);
    process.exit(1);
}
main();
