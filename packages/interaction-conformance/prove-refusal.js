...
function assemble(c) {
    ...
    copyFileSync(join(FIXTURES, c.dir, c.source), join(root, 'src', `${c.dir}-${basename(c.source)}`));
    if (!c.noDist) {
        ...
        if (c.distFrom) {
            ...
            for (const f of readdirSync(from))
                copyFileSync(join(from, f), join(dist, f));
        }
        else {
...
function reportedRules(out) {
    const ids = new Set();
    for (const m of out.matchAll(/^(P\d+) — /gm))
        ids.add(m[1]);
...
for (const c of cases) {
    ...
    if (c.noDist) {
        if (code !== 1)
            problems.push(`expected exit 1 on missing dist/, got ${code}`);
        ...
        if (code !== 0) {
            problems.push(`expected exit 0, got ${code}; reported ${JSON.stringify(reportedRules(out))}`);
        }
    }
    else {
        if (code !== 1)
            problems.push(`expected exit 1, got ${code}`);
    ...
    if (problems.length === 0) {
        passes++;
        const label = c.expect === null ? 'exit 0' : c.noDist ? 'exit 1 + names dist/' : `exit 1 + ${c.expect.join(',')}`;
...
//# sourceMappingURL=prove-refusal.js.map