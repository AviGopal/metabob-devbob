 ...
 */
import { findRegions, findMatching, lineOf, codeIdentifiers, jsxExpressions, } from './scan.ts';
// ---------------------------------------------------------------------------
// Shared helpers
// ---------------------------------------------------------------------------
const TSX = (f) => f.ext === '.tsx';
const TS_TSX = (f) => f.ext === '.ts' || f.ext === '.tsx';
const LIVENESS_RE = /\b(refetchInterval|EventSource|setInterval)\b/;
function hasLiveness(f) {
    return LIVENESS_RE.test(f.code);
}
function firstLineMatching(f, re) {
 ...
 * or `)` (function body).
 */
function statementRegion(code, fromIdx) {
    ...
    while (true) {
        ...
        if (isBody) {
            ...
            if (close !== -1)
                return { start: braceIdx, end: close };
            break;
        }
        ...
        if (close === -1)
            break;
        if (close > limit && semiIdx !== -1 && braceIdx > limit)
            break;
        search = close + 1;
    }
 ...
 */
function secondCallbackParam(body) {
            ...
            while ((k = keyRe.exec(region.body)) !== null) {
                ...
                if (ids.has(indexVar) && ids.size === 1) {
                    out.push({
                        ...
                        hint: `Key on a stable domain id (e.g. key={item.id} / key={run.dispatchId}) instead of the ` +
        ...
        for (const region of findRegions(f.code, /\.(?:sort|toSorted)\s*\(/)) {
            ...
            if (volatile.length > 0) {
                out.push({
                    ...
                    file: f.path,
                    line: region.line,
                    hint: `A comparator must not read a value that changes during a run (${volatile.join(', ')}) — ` +
...
const p7 = (ctx) => {
    ...
    for (const f of ctx.sources.filter(TSX)) {
        ...
        while ((m = re.exec(f.code)) !== null) {
            ...
            const imported = new RegExp(`import[^;]*\\b${root}\\b[^;]*from`, 's').test(f.code);
            if (!imported) {
                out.push({
                    ruleId: 'P7',
        ...
        if (agree) {
            out.push({
                ...
                hint: `An agree-affordance collects assent, not information — replace it with mutually ` +
...
const p9 = (ctx) => {
    ...
    for (const f of ctx.sources.filter(TS_TSX)) {
        for (const region of findRegions(f.code, /\bswitch\s*\(/)) {
            ...
            if (!/\b\w*(?:[Ff]orm|[Ss]hape)\w*\b/.test(discriminant))
                continue;
            ...
            if (nextCase !== -1)
                defaultBody = defaultBody.slice(0, nextCase);
            ...
            if (P9_BLANK_DEFAULT.test(defaultBody)) {
                out.push({
                    ...
                    file: f.path,
                    line,
                    hint: `An unknown content form must fall through to a VERBATIM renderer — 'default: return null' ` +
...
export const RULES = [
    {
        ...
        statement: 'A verdict and a status are never rendered at equal prominence, and status is never rendered alone.',
        tier: 'static-hard',
        checks: 'A .tsx that reads `.status` in a JSX expression must also reference `reached` somewhere in the same file.',
        notCovered: 'Relative prominence. Both symbols can be present and the verdict still rendered as 10px grey text beside a status pill. That is the probe’s verdict-legibility assertion.',
        hint: 'Render the honest `reached` verdict alongside the template exit `status` (status alone is not evidence), or add `// @interaction:exempt P1 — <reason>` on the preceding line.',
        matcher: p1,
    },
    {
        id: 'P2',
        ...
        tier: 'static-hard',
        checks: 'Inside a declaration whose name contains `starter` or `suggestion`, reject `dispatch*(` / `submit*(` / `mutate*(`, `use*Dispatch(Mutation)?`, and `requestSubmit()`.',
        notCovered: 'Indirection. A starter that calls a locally-aliased helper, or dispatches through a context object under any other name, escapes this check entirely.',
        hint: 'A starter must only set input state (e.g. `setInput(text)`) and leave the send to the reader, or add `// @interaction:exempt P2 — <reason>` on the preceding line.',
        matcher: p2,
    },
    {
        id: 'P3',
        ...
        tier: 'static-hard',
        checks: 'A file with a liveness source (`refetchInterval` / `EventSource` / `setInterval`) that renders a `.map(` list must render it through a `<LiveList>` primitive carrying an explicit `buffer=` prop.',
        notCovered: 'Whether the buffer does anything. `buffer={() => {}}` or `buffer={0}` passes. Only the probe’s layout-stability run under a simulated arrival stream can decide this.',
        hint: 'Render live rows through `<LiveList buffer={…}>` so arrivals queue behind a “N new” affordance instead of splicing under the reader, or add `// @interaction:exempt P3 — <reason>` on the preceding line.',
        matcher: p3,
    },
    {
        id: 'P4',
        ...
        tier: 'static-easy',
        checks: 'Captures the SECOND callback parameter of `.map`/`.flatMap` by name — whatever it is called — and rejects `key={IDX}`, `key={`…${IDX}…`}`, and `key={IDX + 1}` inside that callback.',
        hint: 'Key on a stable domain id (`key={item.id}`, `key={run.dispatchId}`) rather than the callback index, or add `// @interaction:exempt P4 — <reason>` on the preceding line.',
        matcher: p4,
    },
    {
        id: 'P5',
        slug: 'stable-comparator',
        statement: 'Every sort comparator ends in a unique tiebreaker, and no comparator reads a value that changes during a run.',
        tier: 'static-hard',
        checks: 'Inside a `.sort(`/`.toSorted(` callback: reject volatile fields (`status`, `updatedAt`, `progress`, `elapsed`, `lastSeen`, `iteration`, `poolSize`); otherwise require ≥2 compared fields or a comparison on `id`/`dispatchId`/`traceId`.',
        notCovered: 'Heuristic on both arms. The field list is a denylist, not a proof of stability, and a comparator can be tie-broken correctly and still be fed a changing value at the call site.',
        hint: 'Compare a value fixed at creation and finish on a unique id (`a.id.localeCompare(b.id)`), or add `// @interaction:exempt P5 — <reason>` on the preceding line.',
        matcher: p5,
    },
    {
        id: 'P6',
        ...
        tier: 'static-hard',
        checks: 'A file with a liveness source must also bind both a `paused`/`isPaused` and an `interval`/`refreshInterval` identifier. (Word-bounded, so `refetchInterval` and `setInterval` cannot satisfy it themselves.)',
        notCovered: 'Whether the controls are wired or reachable. Two unused `useState` bindings satisfy this check. Freeze-on-interaction is the probe’s job.',
        hint: 'Expose reader-controlled `paused` and `interval` state for the live region, or add `// @interaction:exempt P6 — <reason>` on the preceding line.',
        matcher: p6,
    },
    {
        id: 'P7',
        slug: 'declared-mece-options-no-agree',
        statement: 'Feedback options are mutually exclusive and collectively exhaustive, and no agree-affordance is offered.',
        tier: 'static-hard',
        checks: '`options=` must bind an imported identifier rather than an inline array/object literal; separately, agree-shaped labels (`agree`, 👍, `thumbs up`, `looks good`, `lgtm`) are rejected anywhere in a .tsx.',
        notCovered: 'MECE-ness itself. Whether the declared set actually partitions the outcome space is semantic and undecidable here — declaration only makes the set reviewable in one place.',
        hint: 'Import the verdict option set from a single declared module and drop any agree-affordance (assent is not information), or add `// @interaction:exempt P7 — <reason>` on the preceding line.',
        matcher: p7,
    },
    {
        id: 'P8',
        ...
        tier: 'static-hard',
        checks: 'Inside an `<EvidenceSlot>`, violation when every expression child matches `chars|length|traceId|trace_id|dispatchId|sizeBytes` and no `content|preview|body|text` reference appears in the slot.',
        notCovered: 'Semantics. A slot rendering `{summary}` passes while showing a model-authored gloss instead of the payload; the check cannot tell content from a description of content.',
        hint: 'Render the payload itself first and let the length or id sit beside it as provenance, or add `// @interaction:exempt P8 — <reason>` on the preceding line.',
        matcher: p8,
    },
    {
        id: 'P9',
        slug: 'form-switch-falls-through-verbatim',
        statement: 'Rendering dispatches on content form, and an unknown form falls through to verbatim.',
        tier: 'static-easy',
        checks: 'The switch over the content-form discriminant must have a `default:`, and that default must reference a verbatim renderer and must NOT `return null` / `return <></>`.',
        hint: 'Fall through to a verbatim renderer that prints the raw payload — `default: return null` is exactly the silent blank this rule exists to prevent — or add `// @interaction:exempt P9 — <reason>` on the preceding line.',
        matcher: p9,
    },
    {
        id: 'P10',
        slug: 'acceptance-and-stall-are-states',
        statement: 'Acceptance is rendered distinctly from completion, and silence is rendered as stalled.',
        tier: 'static-hard',
        checks: 'An exported run-state union (type name matching `state`/`status`) must contain the literals `accepted` AND `stalled`.',
        notCovered: 'Rendering. The union can carry both literals while the component renders `accepted` identically to `completed` and never derives `stalled` from silence at all.',
        hint: 'Add `\'accepted\'` and `\'stalled\'` to the exported run-state union and render each distinctly, or add `// @interaction:exempt P10 — <reason>` on the preceding line.',
        matcher: p10,
    },
    {
        id: 'P11',
        ...
        tier: 'static-easy',
        checks: 'Over `**/*.{ts,tsx,css}`: `#`-anchored hex, `rgb()/rgba()/hsl()/hsla()/oklch()/lab()/color-mix()`, and Tailwind arbitrary `-[#rrggbb]`. Bare hex-shaped ids (`\'994b5e\'`) and URL fragments (`#section`) do not trip it.',
        hint: 'Use a semantic state token from `@avigopal/design-tokens` (`var(--state-reached)`, …) — `reached` and `not-reached` are defined at equal weight there and a hand-written hex breaks that pairing — or add `// @interaction:exempt P11 — <reason>` on the preceding line.',
        matcher: p11,
    },
    {
        id: 'P12',
        ...
        tier: 'static-easy',
        checks: 'Scans BUILD OUTPUT `dist/**/*.{js,css,html}` for absolute URLs, protocol-relative hosts, and remote `@font-face` `url()`. `http://www.w3.org/*` (SVG xmlns), sourcemap pragmas, and license banners are allowlisted. A missing `dist/` is a HARD ERROR, never a skip.',
        hint: 'Inline or vendor the asset into the bundle, or add `// @interaction:exempt P12 — <reason>` on the preceding line in the source that emits it.',
        matcher: p12,
    },
    {
        id: 'P13',
        ...
        tier: 'static-easy',
        checks: 'Rejects a `%` on any line binding `confidence`/`certainty`/`probability`, and `toFixed(` on such a binding.',
        hint: 'Planner confidence is uncalibrated in this system — conf 0.0 has outperformed conf 0.9 — so a percentage launders a known-bad signal into a precise-looking number. Render the evidence (the trace, the prior outcomes) instead, or add `// @interaction:exempt P13 — <reason>` on the preceding line.',
        matcher: p13,
    },
];
...
export const P0 = {
    ...
    statement: 'An exemption names a real rule and carries a reason. A bare or misdirected exemption is itself a violation.',
    tier: 'static-easy',
    checks: 'Every `@interaction:exempt` / `@interaction:exempt-file` annotation must name a rule id present in this table and carry a reason of at least 12 non-whitespace characters.',
    hint: 'Write `// @interaction:exempt <ruleId> — <reason ≥12 chars>` naming a real rule, or delete the annotation and fix the code.',
};
