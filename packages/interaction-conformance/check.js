...
function main() {
    ...
    if (sources.length === 0) {
        console.error(`interaction-conformance: no .ts/.tsx/.css files found under ${root}.\n` +
    ...
    if (!noDist) {
        if (!existsSync(distRoot)) {
            console.error(`interaction-conformance: build output not found: ${distRoot}\n\n` +
    ...
    // Run the rule table
    // -------------------------------------------------------------------------
    const ctx = { root, sources, distFiles };
    const raw = [];
    ...
    if (findings.length === 0) {
        ...
        console.log(`interaction-conformance: OK — ${scope}, ${activeRules.length} rules, no violations.`);
        process.exit(0);
    }
