...
// File walking
// ---------------------------------------------------------------------------
export function walkFiles(root, exts, opts = {}) {
...
export function commentModeFor(ext) {
    if (ext === '.css')
        return 'css';
    if (ext === '.html' || ext === '.htm')
        return 'html';
 ...
 */
export function stripComments(src, mode = 'js') {
    const out = src.split('');
    const blank = (from, to) => {
 ...
 */
export function findRegions(code, re) {
 ...
 */
export function codeIdentifiers(expr) {
    let cleaned = '';
    let i = 0;
    while (i < expr.length) {
        const c = expr[i];
 ...
 */
export function jsxExpressions(body) {
    const out = [];
    let i = 0;
    while (i < body.length) {
        if (body[i] === '{') {
