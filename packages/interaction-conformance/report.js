 ...
 */
import { RULES_BY_ID, P0 } from './rules.ts';
export function report(findings, root) {
    console.error(`interaction-conformance: ${findings.length} violation(s) found in ${root}\n`);
    ...
    for (const ruleId of order) {
        ...
        if (!group || group.length === 0)
            continue;
        const rule = ruleId === 'P0' ? P0 : RULES_BY_ID.get(ruleId);
        console.error(`${rule.id} — ${rule.slug.toUpperCase().replace(/-/g, ' ')} (${group.length}) [${rule.tier}]:`);
    ...
    console.error(`immediately preceding the flagged line (or '// @interaction:exempt-file <ruleId> — <reason>' in the`);
    console.error(`first 5 lines for a whole file). A bare exemption, or one naming an unknown rule, is itself a P0 violation.`);
    console.error(`Run with --list to print the rule table, or --emit-concepts to regenerate the concept-db lesson payloads.`);
}
export function printRuleTable() {
    const rows = [
    ...
    for (const [id, slug, tier, statement] of rows) {
        ...
        if (rule?.notCovered)
            console.log(`     NOT COVERED: ${rule.notCovered}`);
        console.log('');
    }
}
